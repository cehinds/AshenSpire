...
- **The Armoury opens on your figure, and CARDS is one click away** ([#316](https://github.com/cehinds/AshenSpire/pull/316), `0.4.0.0983`). The card strip now arrives folded, on every shape, so the character you dressed is whole the moment the panel opens instead of being squeezed into a scrolling sliver by the cards beneath it. One click on CARDS opens the strip, another folds it again, and whatever you leave it on is what the Armoury gives you next time. On a phone nothing changes: that view never showed the figure and already opened folded.
