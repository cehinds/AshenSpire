...
- **The build stamp now says which release a build is a candidate for: `0.5.0-rc.1`** ([#517](https://github.com/cehinds/AshenSpire/pull/517), `0.5.0-rc.1.1906`). The number a tester reads finally reflects the framework milestone in flight: builds on this line are release candidates for 0.5.0, ordered below the final 0.5.0 that the cutover gate still has to earn. Saved runs carry over. Also repairs the three CI instruments that kept the hosted board red (#498, #499): the About-changelog check can find a browser on Linux, the link-check selftest has heap room on macOS, and the version check no longer trips on a prose-valued manifest key. Release remains RED.
