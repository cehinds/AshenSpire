...
try{
 await page.goto('http://127.0.0.1:4291/transition-playback-preview.html');
 for(const id of Object.keys(SEQUENCES)){
  ...
  const order=SEQUENCES[id].order||[0,1,2,3,4,5,6];
  await page.locator('[data-frame="0"]').click();const actual=[];
  for(let i=0;i<order.length;i++){actual.push(await page.evaluate(()=>window.transitionReference.frame));await page.locator('#next').click()}assert.deepEqual(actual,order);
  if(id==='reaver')assert.deepEqual(actual.map(i=>i+1),[1,3,2,6,5,4,6,7,1]);
  await page.locator('#back').click();assert.equal(await page.evaluate(()=>window.transitionReference.frame),order.at(-1));await page.locator('#next').click();assert.equal(await page.evaluate(()=>window.transitionReference.frame),0);
  ...
  await page.screenshot({path:`${root}/inspection/${id}-desktop.png`,fullPage:true});playback.push({id,order:actual.map(i=>i+1),framesSeen:[...seen].sort(),quarterSpeed:true,durationMs:order.reduce((sum,i)=>sum+DURATIONS[i],0)});
 }
 ...
 await page.goto('http://127.0.0.1:4291/transition-playback-preview.html?class=invalid');assert.equal(await page.evaluate(()=>window.transitionReference.id),'reaver');
