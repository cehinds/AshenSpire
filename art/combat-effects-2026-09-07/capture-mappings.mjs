...
try{
 ...
 await call('Page.navigate',{url:'http://localhost:4290/art/combat-effects-2026-09-07/current-tag-mappings.html'});
 ...
 if(result.result.value.length!==6||decoded.result.value<300)throw Error('Mapping page did not load completely');
 for(const [i,clip] of result.result.value.entries()){
  ...
  writeFileSync(new URL('inspection/current-tag-mappings-'+(i+1)+'.png',import.meta.url),Buffer.from(shot.data,'base64'));
 }
