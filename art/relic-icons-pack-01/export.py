...
for index, item in enumerate(items):
    ...
    cards.append(f'<article><div class="hero"><img src="webp/{rid}-512.webp" alt="{safe}"></div><h2>{safe}</h2><code>{rid}</code><p>Starter relic · {html.escape(item["acquisition"]["className"])}</p><blockquote>{html.escape(item["lore"])}</blockquote><dl><dt>Acquisition</dt><dd>{html.escape(item["acquisition"]["method"])}</dd><dt>Starting grant</dt><dd>{html.escape(item["acquisition"]["startingGrant"])}</dd><dt>Normal reward drop rate</dt><dd>{html.escape(item["acquisition"]["normalRewardDropRate"])}</dd><dt>Boss reward drop rate</dt><dd>{html.escape(item["acquisition"]["bossRewardDropRate"])}</dd><dt>Boss ownership</dt><dd>{html.escape(item["acquisition"]["bossOwner"])}</dd></dl><div class="sizes"><img width="48" height="48" src="webp/{rid}-64.webp" alt="{safe} at 48 pixels"><img width="64" height="64" src="webp/{rid}-64.webp" alt="{safe} at 64 pixels"><img width="128" height="128" src="webp/{rid}-128.webp" alt="{safe} at 128 pixels"></div><p>48 / 64 / 128 px</p><details><summary>Art interpretation</summary><p>{html.escape(item["subject"])}</p></details><a href="sources/{rid}.png">Original PNG</a> · <a href="prompts/{rid}.txt">Exact prompt</a><p>' + ' · '.join(f'<a href="webp/{rid}-{n}.webp">{n}px WebP</a>' for n in (512,256,128,64)) + '</p></article>')
