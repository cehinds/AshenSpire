...
def extract_figures(atlas, count=15, threshold=32):
    ...
    # Vectorized bounds avoid scanning the whole atlas once per tiny edge component.
    ys,xs=np.nonzero(labels)
    pixel_ids=labels[ys,xs]
    x0=np.full(len(parents),atlas.width,dtype=np.int32)
    y0=np.full(len(parents),atlas.height,dtype=np.int32)
    x1=np.zeros(len(parents),dtype=np.int32)
    y1=np.zeros(len(parents),dtype=np.int32)
    np.minimum.at(x0,pixel_ids,xs);np.minimum.at(y0,pixel_ids,ys)
    np.maximum.at(x1,pixel_ids,xs+1);np.maximum.at(y1,pixel_ids,ys+1)
    components=[{'id':int(label),'area':int(area),'box':(int(x0[label]),int(y0[label]),int(x1[label]),int(y1[label]))} for label,area in zip(ids,areas)]
