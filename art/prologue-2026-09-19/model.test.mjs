...
test('all edits survive a JSON round trip, including distinct class dialogue',()=>{const draft=structuredClone(defaults);draft.scenes[0].text='A new line.\nAnd another.';draft.classes.reaver.line='Keep the gate.';draft.classes.herald.line='Keep the book.';draft.labels.setForth='Take the road';draft.presentation.tintSource='character';draft.presentation.characterTint='frost';draft.presentation.wash=.28;const imported=validateSequence(JSON.parse(JSON.stringify(draft)),defaults);assert.deepEqual(imported,draft);assert.equal(sceneCopy(imported.scenes[3],imported,'reaver').text,'Keep the gate.');assert.equal(sceneCopy(imported.scenes[3],imported,'herald').text,'Keep the book.');assert.equal(motifColour(imported),'#7fa8c9');});
test('invalid imports do not mutate existing state and cannot override artwork paths',()=>{for(const edit of [d=>d.scenes[0].seconds=-1,d=>d.scenes[1].id='warmth',d=>d.presentation.accent='unknown',d=>d.classes.rogue.line=null,d=>d.presentation.transitionSeconds=Infinity]){const draft=structuredClone(defaults),before=JSON.stringify(defaults);edit(draft);assert.throws(()=>validateSequence(draft,defaults));assert.equal(JSON.stringify(defaults),before);}const draft=structuredClone(defaults);draft.scenes[3].actor.desktop.height=999;draft.palettes.accent.gold='url(javascript:bad)';const imported=validateSequence(draft,defaults);assert.equal(imported.scenes[3].actor.desktop.height,defaults.scenes[3].actor.desktop.height);assert.equal(imported.palettes.accent.gold,defaults.palettes.accent.gold);});
test('empty and non-ASCII authored text remains editable',()=>{const d=structuredClone(defaults);d.scenes[2].speaker='';d.scenes[2].text='Cendres — 灰\n';assert.equal(validateSequence(d,defaults).scenes[2].text,d.scenes[2].text);});
