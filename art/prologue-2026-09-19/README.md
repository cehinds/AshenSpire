...
`../../assets/prologue/` contains twenty background WebPs (including two retained earlier carry plates) and four alpha-preserving traveller WebPs. New desktop plates are 1280 pixels wide and portrait plates 768, encoded from PNG masters at WebP quality 82, method 6. `assets.json` records current sizes and dimensions. All files decode; PNG masters and duplicate ZIPs are not required by the game build.
