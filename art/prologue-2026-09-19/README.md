...
`../../assets/prologue/` contains twenty background WebPs (including two retained earlier carry plates and the two `road-*` plates of the cut scene, which the game no longer loads) and four alpha-preserving traveller WebPs. New desktop plates are 1280 pixels wide and portrait plates 768, encoded from PNG masters at WebP quality 82, method 6. `assets.json` records current sizes and dimensions. All files decode; PNG masters and duplicate ZIPs are not required by the game build.
...
The hamlet paintings show the Hollow Weald departure. The final scene keeps its own painting at every start and names the destination in its caption; it no longer swaps in Crownfall’s or a region’s map art. Class never determines geography. The neighbour’s burning is unfinished; the player is not universally transformed. Custom body/armour appearances are not represented by the four class layers.
