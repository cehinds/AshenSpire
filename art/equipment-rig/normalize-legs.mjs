...
const parts=['pelvis','nearThigh','nearShin','nearBoot','frontSkirt','farThigh','farShin','farBoot','torsoCrouch','torsoOverhead','cloakAction','backSkirt'];
...
for(const id of ['reaver','starseer']){
 const {data,info}=await sharp(`${base}/masters/${id}-legs.png`).ensureAlpha().raw().toBuffer({resolveWithObject:true});
 ...
 await mkdir(`${base}/parts/${id}`,{recursive:true});await writeFile(`${base}/parts/${id}/legs-atlas.png`,normalized);
 const bounds={};
 for(let i=0;i<12;i++){
  ...
  const rect={left:minX,top:minY,width:maxX-minX+1,height:maxY-minY+1};// Atlas thighs included some shin and shins included boots; retain only the
  // intended segment before assigning bone attachment landmarks.
  if(parts[i].endsWith('Thigh'))rect.height=Math.round(rect.height*.62);
  if(parts[i].endsWith('Shin'))rect.height=Math.round(rect.height*.77);
  bounds[parts[i]]=rect;
  await sharp(normalized).extract(rect).png().toFile(`${base}/parts/${id}/${parts[i]}.png`);
 }
 await writeFile(`${base}/parts/${id}/leg-bounds.json`,JSON.stringify(bounds,null,2));console.log(id,12+' parts',count+' transparent pixels');
}
