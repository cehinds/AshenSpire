...
function draw(){if(!textures)return;let max=0,scene;for(const id of ['reaver','starseer']){const s=render($(id),textures,id,select.value,$('action').value,Number($('time').value)/100,{debug:$('debug').checked,effects:$('effects').checked,guided:$('motionModel').value==='traced'});max=Math.max(max,s.right.error,s.left.error,s.nearLeg.error,s.farLeg.error);scene=s;}$('phase').value=$('time').value+'%';$('status').textContent=`${scene.pose.from.label} → ${scene.pose.to.label} · Grip target error: ${max.toFixed(1)} px · Front foot: ${scene.pose.front.planted?'planted':'stepping'}`;for(const card of document.querySelectorAll('.reference-card'))card.classList.toggle('active',Number(card.dataset.time)===Number($('time').value));}
...
async function showReferences(){if(!textures)return;const version=++referenceVersion;const setupId=select.value,action=$('action').value,keys=referenceKeys('reaver',WEAPONS[SETUPS[setupId].main].family,action).filter(k=>[.22,.46,.69].includes(k.t));$('references').replaceChildren();$('referenceTitle').textContent=keys[0].referenceClass==='starseer'?'Starseer casting pose guides':'Reaver pose guides';
 const cards=await Promise.all(keys.map(async k=>{
  ...
  const source=new Image();source.src=`assets/painted-outfits/${k.referenceClass}/${k.referencePose}.webp`;await source.decode();const rigCanvas=document.createElement('canvas');rigCanvas.width=600;rigCanvas.height=600;render(rigCanvas,textures,k.referenceClass,setupId,action,k.t,{effects:false,guided:$('motionModel').value==='traced'});
...
for(const id of ['time','debug','effects','rigSpeed','motionModel'])$(id).addEventListener('input',()=>{stop();draw()});
$('motionModel').onchange=()=>{stop();draw();showReferences().catch(report)};
$('reaverPreset').onclick=()=>{select.value='greatsword';choose()};$('casterPreset').onclick=()=>{select.value='focus';choose()};
$('play').onclick=()=>{if(playing){stop();return}$('time').value=0;draw();playing=true;$('play').textContent='Pause';const start=performance.now(),duration=2200/Number($('rigSpeed').value);const tick=now=>{$('time').value=Math.min(100,(now-start)/duration*100);draw();if(now-start<duration)request=requestAnimationFrame(tick);else stop()};request=requestAnimationFrame(tick)};
