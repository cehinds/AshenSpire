...
for(const setup of ['sword','greatsword','shield','focus','mixed','dualCast','dual','newWeapon']){
 await page.selectOption('#setup',setup);for(const phase of ['0','46','69','100']){await page.locator('#time').fill(phase);assert.match(await page.locator('#status').innerText(),/0.0 px/)}
}
// Review both puppets through representative weapon-family moments.
for(const id of ['reaver','starseer']){
 const cells=[];let index=0;
 for(const [setup,action] of [['greatsword','attack'],['shield','guard'],['focus','cast'],['newWeapon','attack']])for(const t of [.2,.46,.69]){
...
await page.selectOption('#setup','greatsword');await page.locator('#time').fill('46');await page.screenshot({path:dir+'/desktop.png',fullPage:true});
...
assert.deepEqual(errors,[]);await writeFile(dir+'/checks.json',JSON.stringify({setups:8,classes:2,phasesPerSetup:4,visualFrames:24,clippedFrames:0,browserErrors:errors,viewports:[1280,390],playback:'completed',tooling:'Edge via Playwright'},null,2));await browser.close();console.log('8 setups, 24 inspected-frame captures, controls, playback, desktop and mobile passed.');
