...
export async function runTests({ artManifest = null, assetExists = null, legacyRunSave = null, preE6RunSave = null } = {}) {
  ...
  test('50. five-stat creation vocabulary and class presets come from one complete content product', () => {
    ...
    eq(`${fresh.maxHp}/${fresh.energyMax}/${fresh.drawPerTurn}`, '40/3/3', 'lean HP/actions/hand formulas reach the run, read against the attributes the sheet shows (ruleset 7: 36 + 2 × CON)');
    ...
    eq(`${rogue.attributeMode}/${rogue.maxHp}/${rogue.energyMax}/${rogue.drawPerTurn}`, 'lean/40/3/3', 'Rogue lean stats reach the HP, action, and hand formulas');
  ...
  test('93. the attribute rebase: one creation scale, one derived ruleset, one equip gate (plan phase 9)', () => {
    ...
    eq(rules.rulesetVersion, 7, 'the authored table is ruleset 7 — the one format, retuned for the lean scale (plan A3)');
