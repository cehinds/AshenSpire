...
test('Stats is the one menu for each trait: formulas, hand rules, ratings and their constants', () => {
  ...
  assert.equal(topicOf('gameConfig.handRules.startingByClass.reaver.base'), 'Draw & hand', 'each class opening hand too (owner, 2026-09-24)');
  assert.equal(topicOf('gameConfig.handRules.starting.base'), undefined, 'the shared base is retired: every class sets its own');
...
test('the worked example recomputes from the edited values and shows the whole sum', async () => {
  ...
  const deepSettings = { 'gameConfig.handRules.startingByClass.reaver.base': 20, 'gameConfig.handRules.starting.maximum': 30, 'gameConfig.handRules.capacity.base': 20 };
