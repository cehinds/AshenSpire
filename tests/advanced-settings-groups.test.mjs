...
test('Stats is the single menu for conversions, ratings, and hand rules', () => {
  ...
  assert.ok(!rows.some(row => ['gameConfig.balance.energy', 'gameConfig.balance.draw'].includes(row.key)),
    'obsolete resource constants are not exposed as competing controls');
  assert.match(rows.find(row => row.key === 'gameConfig.balance.handMax')?.note || '', /Fallback hand capacity/,
    'the legacy hand-cap fallback is clearly distinguished from active hand rules');
