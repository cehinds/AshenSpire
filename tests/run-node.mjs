                    ...
                    // two-homes defect these two lines exist to catch.
{
  ...
  // ---- 49/50. CAN A PLAYER EVER MEET THIS STATUS? (Rune, 2026-08-08) -------
  //
  ...
  //
  // TWO LINES, for the same reason 45/46 are two: 49 is the check's own
  // integrity against its planted corpus (including the two plants that must
  // go GREEN), 50 is the state of the tree. No plant count typed here — the
  ...
  console.log(
    `${reachSelf.code === 0 && reachSelfV.text ? 'PASS' : 'FAIL'}  49. the status-reach check still catches its own known-bad corpus` +
  ...
  console.log(
    `${reachTree.code === 0 && reachTreeV.text ? 'PASS' : 'FAIL'}  50. every shipped status has something that applies it` +
...
console.log('          49–50 prove a status has a DOOR, never that the door opens on');
