...
- `node --test tests/settings-revamp.test.mjs` (at the time of #1299; retired with the old suite, so these are now covered only as far as `tests/ui-screens.test.mjs` mounts Settings): channel detection, the debug
