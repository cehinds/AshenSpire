...
| `hud-quick-settings` | `hudQuickSettingsModel` | `hudQuickSettingsHtml` | Title + Map + Combat | Shared right-anchored Fullscreen/Music utility rail. Fine-pointer wide screens use data-owned 24px rows; mobile glyph centers close to 28px while their phone/coarse-pointer controls retain separate 44px touch targets. The entrance-to-boss receipt remains in the HUD flow so the rail cannot cover it. |
