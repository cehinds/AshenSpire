...
|------|---------|-----------------|------------------|
| G1 | `node --test "tests/*.test.mjs"` | Exit 0. The five scenario tests pass: solo run, combat rules, co-op, content integrity, UI screens (#1319). | `tests.yml` `tests` job |
| G2 | `node tools/verdict.mjs -- node tools/buildversion.mjs --check` | Exit 0. The build version is derived and matches the tree, and nobody typed it by hand. | `tests.yml` `build` job |
...
| G4 | `node tools/release-series.mjs` | Exit 0. The version series in the tree is the one the owner approved (docs/versioning.md). | `tests.yml` `build` job |
| G5 | `node tools/config-build.mjs --check` | Exit 0. The generated UI config is current with `content/config/`. Run it bare: its "is current with N source file(s)" line is not a form the verdict door accepts, so wrapped it exits 3 on a green tree. | `tests/content.test.mjs` |
| G6 | `node tools/balance.mjs --check` | Exit 0. docs/BALANCE.md matches a fresh run. | none: run by hand (the suite was cut to five tests in #1319) |
| G7 | retired | `tools/workflow-lint.mjs` was retired with the old suite in #1319; GitHub refuses a malformed workflow itself. | — |
| G8 | `node tools/launch.mjs --build-only` | The bundler's parse gate runs inside every build (its standalone fixture suite was retired in #1319). | `tests.yml` `build` job |
| G9 | `node tools/verdict.mjs -- node tools/launch.mjs --build-only` | Exit 0. The standalone builds are regenerated from the RC source. | `tests.yml` `build` job |
| G10 | `node tools/verdict.mjs -- node tools/verify-shipped.mjs` | Exit 0, run after G9. The root and `dist/` copies a player is handed carry art and equal the fresh `build/`. | `tests.yml` `build` job |
...
| G12 | `node tools/verdict.mjs -- node tools/about-changelog.mjs` | Exit 0. The in-game changelog is a faithful projection of CHANGELOG.md, in order. | `receipts.yml` `about-changelog` job |

Gate G13 has no local command. `ci.yml` (the 3-OS matrix) was retired when CI was
collapsed in #1319; until the owner names its replacement (FINISH.md §12, owner
decision D7), the `tests`, `dev-preview` and `receipts` workflow runs on the RC SHA
must conclude **success** on every job. Name the run URLs in the sign-off.
...
Keep this map by hand (its checking test was retired with the old suite in #1319): it
is wrong when a FINISH.md criterion has no row
