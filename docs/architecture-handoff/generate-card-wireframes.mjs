...
function sizes(c,mode){
 ...
 ['buildup','100% component','0.5 × HP bar height','status','top','stretch / center','rows first','0.35rem gap','threshold progress; semantic color and text'],
 ['statusIcons','100% component','1.575rem; exactly one icon row','status','below stance','center / center','last row; final +N opens inspector','0.21rem gap','uniform 1.575rem square tiles; icon only; details in tooltip/inspector'],
 ['stance','100% component','1 × HP bar height','status','below buildup','center / center','before status icons','0.35rem gap','uniform badge dimensions across all stances'],
 ...
 // Reference viewport 1600x1000 wide, 1000x800 compact, 400x800 portrait; width clamped, shared ratio 5:8.
 const w={wide:16,compact:24,portrait:48}[mode],h={wide:40.96,compact:48,portrait:38.4}[mode];
 const head=h*.1,art=h*.4,tags=h*.08,footer=h*.1,body=h*.4;
 const pad=0.8,iw=Number((w-2*pad).toFixed(2));
 const rows=[['frame',w,h,'Nominal card host allocation'],['header',w,head,'Title and optional cost/state; no duplicate title'],['header.title',Number((iw*.65).toFixed(2)),head,'Top-left'],['header.state',Number((iw*.3).toFixed(2)),head,'Top-right if applicable'],['art',w,art,'Aspect-preserving art well'],['art.tags',iw,tags,'Bottom of art band; nested inside art 40%, meaningful tags only'],['body',w,body,'Includes inner spacing and blocker'],['footer',w,footer,'Supplementary metadata only; no embedded action'],['footer.metadata',iw,Number((footer*.8).toFixed(2)),'Rarity left; owned count right when relevant; no embedded command']];
 c.fields.forEach((field,i)=>rows.push(['body.detail'+(i+1),iw,Number(((body-h*.06)/c.fields.length).toFixed(2)),field]));
 rows.push(['body.blocker',iw,Number((h*.06).toFixed(2)),'Shares body budget; omit and reclaim space when absent']);
 rows.push(['selection.outline',w,h,'Overlay follows lifted card perimeter; does not consume band height'],['selection.info',4,4,'External centered info button; minimum target size overrides nominal size']);
 return '| Component ID | Width | Height | Relative to | Anchor | Align X / Y | Positioning | Offset / gap | Content / ownership |\n|---|---:|---:|---|---|---|---|---|---|\n'+rows.map(([slot,a,b,note])=>`| \`${c.id}.${slot}\` | ${a}vw | ${b}vh | ${positioning(c.id,slot,mode,note).join(" | ")} | ${note} |`).join('\n')+'\n\n';
}
function pseudo(c){
