...
`combatant-focus.mjs` supplies defaults; `combatant-focus.js` contains pure calculations. Back/middle/front base factors are 0.9/0.95/1. Selected growth is 1.1/1.05/1.1, read from configuration. Selection gets the highest configured paint priority.
...
2. Group actors by shared size category. Pass the player and enemy allocations together to `sharedCombatantBaseScale(groups, config)`. Each group represents one row; nested front/middle/back arrangements should supply their individual row allocations.
3. Calculate `rowPresentationScale(actor.row, actor.selected, config)`. Final transform scale equals shared category base scale multiplied by its returned factor. Apply returned `zPriority` to the actor's positioning wrapper.
...
FOR actor IN categoryGroups
  ...
  SetPaintPriority(actor, rowStyle.zPriority)
