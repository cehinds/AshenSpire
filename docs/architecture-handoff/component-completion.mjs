...
export const componentCompletionDefaults = {
  ...
  combatantVisibility: { unselectedHiddenSelectors: ['.combatant-nameplate', '.combatant-meter:not(:first-child)', '.buildup', '.stance'] },
  groundGrid: { rowScaleFactors: [0.9, 0.95, 1], preserveInformationSize: true, slotsPerSide: 6, columns: 2, rows: 3, depthPercent: 58, rowStepRem: 4.5, paddingRem: 1, slotGapRem: 1.5, centerGapRem: 3, centerGapPercent: 10, backRowOffsetPercent: 5, actorScale: 0.78, showGuides: true },
...
const rows = [
 ...
 ['WCO5','Ground shadow',['WCI2'],'overlay','    sprite feet\n───────┼─────── ground contact\n     (shadow)','Render a noninteractive ellipse beneath sprite artwork. Its center is the shared ground-contact anchor. Resolve config.groundShadow dimensions and opacity. Preserve this anchor when status rows change; hidden shadow retains its logical pivot.'],
 ...
 ['WGS8','Ground formation grid',[],'stage','Allies · fill left to right         Enemies · fill left to right\n[ A1 ] [ A2 ]       [ E1 ] [ E2 ]\n  [ A3 ] [ A4 ]   [ E3 ] [ E4 ]\n    [ A5 ] [ A6 ] [ E5 ] [ E6 ]\n ┴   ┴   ┴   ┴               ┴   ┴   ┴   ┴\n         shadow centers anchor to ground','Compose two equal ground regions with config.groundGrid fixed padding, slot gaps and center gap. Reserve six slots per side in two columns and three staggered rows, even when empty. Fill row-major from left to right. Row depth comes from config.groundGrid.rowStepRem. Assign stable ordered allies and enemies to slots from the left. Anchor WCO5 centers to slot centers. Clamp counts to slot capacity; preserve empty slots without recentering. Apply config.groundGrid.actorScale.'],
 ['WGS7','Floor layer',['WGS8'],'background','                 scene top\n┌──────────────────────────┐\n│ floor at bottom          │\n└──────── actor baseline ──┘','Paint a foreground ground cutout over the full-bounds skyline using config.scene.groundCutout and config.scene.floorHeightPercent. Skyline remains underneath, visible through transparent cutout areas. Omit when config.scene.floor is disabled without shifting the actor placement model.'],
 ...
 ['WGC4','Target layer',['WCF3'],'stage','Player       [eligible enemy]   unavailable\n                    ↑ active target outline','Use domain-eligible entity IDs for hit regions and keyboard navigation. Highlight eligible and selected targets without selecting impossible targets. Clear stale selection on model change. Use the combatant hit region itself, never an additional Eligible target demo button.'],
