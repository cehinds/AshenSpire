...
export function componentDimensions(s,mode){
 ...
 if(game){
   ...
   if(s.id==='W4b')row('scene.map',95,60,'Owner target ≈60vh ×95vw');
    ...
    const slotWidth=mode==='wide'?slot.widthVw:slot.compactWidthVw;
    ...
    row('scene.playerPortrait',slotWidth,`${figure}vh figure · ${visible}vh visible`,`Layer z4 · full figure zoomed so the top ${share.numerator}/${share.denominator} spans slot top (${slotTop}vh) → reveal line; the rest is occluded by the context band, footer and frame edge, never cropped`);
    row('scene.npcPortrait',slotWidth,`${figure}vh figure · ${visible}vh visible`,'Layer z4 · as the player, mirrored to face the player; the speaker draws above the listener');
