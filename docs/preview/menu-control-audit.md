...
PR #779 replaces resizable Armoury trays with natural-height sections in one scrolling modal. Equipment, Character and Overview retain the game's existing gold, parchment, dark-panel and sprite treatment. Gear selection does not require dragging, and resizing does not close disclosures.
...
- Armoury: all three views; compatible-item filter and clear; list/grid toggle; one-open character accordion; Cards foldout; content width and scrolling; Escape.
...
Full Node suite: 136 passed, 0 failed. Final Armoury focus and layout checks passed at 1440, 390 and 320 pixels. Generated build 115 passed buildversion and verify-shipped checks.
