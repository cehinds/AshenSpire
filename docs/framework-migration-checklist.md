...
| F-18 | Promote `rc.3` to `test` | DONE | #557 `dev → test` merged at `3726fe26` from `dev` @ `16fc2cf2`, stamp `0.5.0-rc.3.1948`, digest `b4a108c043`; span 6 pull requests, 27 commits, 19 files. The hosted board (`ci.yml` run 303) was green on that exact head — `tests` on ubuntu, macos and windows, the shipped-artifact-equals-source job on all three, and the browser gates. Codex's Gate C question (decision 0009 asks a true fast-forward; `test` is not an ancestor of `dev`, so this landed as a merge as #528 and #541 did) is recorded on the pull request for the owner |
