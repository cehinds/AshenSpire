...
| 17 | Complete validation + known-bad corpus | LIVE | `validate.js`, 73 framework tests (`node tests/framework.test.mjs` at this commit) + legacy 114 | — |
...
## F. The `0.5.0` candidates — what was asked, what was done (as of `dev` @ `fe43d594`, 2026-09-02)
...
| F-10 | Promote `rc.2` to `test` | OPEN — #541 stands against `test`, mergeable | its packet is written for `9aeebeba` and is refreshed to the exact `dev` head, ordinal, digest and span before it merges; merged by an agent once Codex is clean on the head |
| F-11 | The owner's `release` cut for `rc.2` | DONE (OWNER) | #542 `dev → release` merged by the owner at `00330f4f` |
| F-12 | Review findings that arrived after the owner merged #549 and #550 | OPEN — follow-up PRs | a seat held out of a fight must not turn the fighters' defeat into rewards; a lethal replay's result is shown before the seat leaves the queue; the text read classifies ancestors met at text points and reads an overlapping in-flow child from the paint order |
| F-13 | CHANGELOG receipts for #543–#550 | OPEN | written with the #541 refresh (`node tools/about-changelog.mjs --write`, rebuild) |
| F-14 | The owner's feedback file, the open-issue sweep (task 34) | PARTIAL | sections D and E stand as written; the sweep resumes after the `rc.2` close-out |

What was **not** done, and why: no agent merged `dev → release` (#529, #542 — the owner's act by the standing rule); the `dev → test` merge of `rc.2` waited on the review program above so that one packet describes one exact `dev` head; the framework gate's row 18 stays NOT PASSED by the rulings' design (A-18).
