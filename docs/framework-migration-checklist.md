...
| E13 | too much battling; level at rest sites with cinders, 1 point/level, 10–20 level-ups a run; rest-stop placement guard rails; multi-use rest toggle | **DONE in #522** (merged) — the sim measures level-ups per full run; the shipped 800 + 200 ladder gave 0.5, the calibrated 20 + 4 gives 14.8 at the greedy ceiling and a default `node tools/runsim.mjs` fleet reads **11.8 per run over 120 runs, 14.7 over the 53 victorious ones**, inside his 10–20; multi-use Shrines toggle (default off) with LEAVE. Shrine-before-boss was already live | #258 **still open**, `state:in-flight` | nothing owed in code. The evidence is recorded on the issue for the owner to close |
...
| F-18 | Promote `rc.3` to `test` | **MERGED, GATE C EXCEPTION** | #557 `dev → test` merged at `3726fe26` from `dev` @ `16fc2cf2`, stamp `0.5.0-rc.3.1948`, digest `b4a108c043`; span 6 pull requests, 27 commits, 19 files; rollback target `2d34d446` (`0.5.0-rc.2.1941`, digest `962538f901`). The hosted board (`ci.yml` run 303) was green on that exact head — `tests` on ubuntu, macos and windows, the shipped-artifact-equals-source job on all three, and the browser gates. **Not a clean Gate C pass**: condition 3 ("current `test` is an ancestor of that SHA, so the mutation is a true fast-forward") is FAIL — `test`'s tip is #541's merge commit, so the merge-base is `9e32f1ac` and no fast-forward exists. 0009 says a FAIL condition means `WAIT` and a request for the exact additional authority; the authority actually relied on was the owner's standing directive that `dev → test` promotion is agent-mergeable, plus the precedent of #528 and #541, which landed as merges for the same reason. That is an owner instruction, not a recorded Gate C authorization, and this row says so rather than reading DONE. See F-20 |
| F-19 | The owner's `release` cut for `rc.3` | OWNER | never merged by an agent |
| F-20 | Reconcile decision 0009's Gate C with three promotions of practice | OPEN — OWNER | The fast-forward property Gate C requires was lost when #528 landed as a merge; #541 and #557 followed. Two ways out, and the choice is the owner's: amend Gate C to describe merge-based promotion (the exact `dev` SHA is still in the promoted ancestry and the promoted tree is byte-identical to `dev`'s, so the acceptance evidence Gate D reads is unaffected), or restore the fast-forward by resetting `test` to the `dev` SHA — which discards the merge commits recording #528, #541 and #557, and is a history rewrite on a shared branch that no agent performs |

What was **not** done, and why: no agent merged `dev → release` (#529, #542 — the owner's act by the standing rule, and the `rc.3` cut is his too); the `dev → test` merge of `rc.2` waited on the review program above so that one packet described one exact `dev` head; the framework gate's row 18 stays NOT PASSED by the rulings' design (A-18); and the owner's feedback file and open-issue sweep (F-14) is SWEPT with nothing owed in code — what is left there is his to close, not an agent's.
