...
- [ ] **Builds are not committed; CI builds them** (owner, 2026-09-26; contract in `docs/versioning.md`, *Builds are not committed*). Test: `git ls-files AshenSpire.html AshenSpire-mobile.html build/ dist/ | grep -c '\.html$'` prints 0 on `dev`; a PR into `dev` changes no generated build file; and a push to `test` produces a `test-standalone-<commit>` artifact that opens and plays, with the Settings → About stamp equal to that commit's `buildordinal.json`.
