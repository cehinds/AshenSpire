...
- [ ] **Remove the stale Frostbite deviation**: SPEC §4.4 marks Frostbite CUT, with Frost and `frostExposed` carrying it. Test: DEVELOPER.md "M1 known deviations" no longer numbers Frostbite among its rows (only a note that it is CUT, not deferred), and the SPEC falsifier `statuses.some(s=>s.id==='frostbite')` prints `false`.
