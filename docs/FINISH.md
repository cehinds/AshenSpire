...
- [ ] **Shop kinds and the guaranteed minimum** (§14.2, step 4). Test: `tests/shop-kinds.test.mjs` covers the following: every kind rolls at least `guaranteedMinimum` offerings over 200 seeds with every chance set to 0; a `guaranteedMinimum` of 1 is refused by name; a disabled offering never appears; a pre-§14 `run.shopStock` loads as `market` unchanged; the classic merchant's existing shelves are byte-identical on 50 fixed seeds.
- [ ] **Market additions** (§14.3, step 5). Test: stones, armour and inn rest can be bought and persist across a reload; a skill book pays its XP through `awardSkillXp`; a revive token spends once at 0 HP and the saved fight replays it; a companion's `combatsLeft` counts down and it leaves at 0; an installed rune's trigger fires only while its piece is equipped.
- [ ] **Blacksmith screen** (§14.4, step 6). Test: refining spends exactly `refine.from` stones; a rune slot is bought, filled and emptied; a stacked copy is a new `upgraded: false` instance the editor counts as owned; a granted card or a Strike cannot be stacked.
- [ ] **Wise master** (§14.5, step 7). Test: a level-4 track respecs to level 1 with xp 0 and the pool gains `floor(pct × spent)`; level 1 is refused; `respecRefundPct` 80 clamps to 75; a master with 2 skills is refused by name; training and redistribute level a track through the one writer.
