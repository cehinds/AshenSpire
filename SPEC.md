...
- **The level's own term** rides the derived-stat rows (`content/derivedStats.js` `perLevel: { every, gain }`, optional per row, refused by name when malformed): `deriveStat` adds `floor((level − 1) / every) × gain` at the character level it is handed. Settings presents the equivalent, simpler `floor(level multiplier × levels after 1)` and stores a positive multiplier as `{ every: 1 / multiplier, gain: 1 }`; zero stores a zero gain. HP gains 1 per level after the first, Mana and Stamina gain 0.2 per level, and Draw gains 0.1. Every derivation of a run's pools (birth, the load door's integrity check, `reconcileRunLoadoutHp`, the stat projection) passes the run's level. The term is snapshotted with the row: a run born under it keeps it whatever the table says later, and a run born before it never gains it, so no old save is re-priced or refused. Legacy interval/gain profiles retain their exact stepped behavior until that trait's multiplier is edited.
...
*Falsify:* a bundle whose `tuned2` preset misses the mode total, or whose ruleset omits a row the version requires, is refused by name; a run created under the rebase opens with Wisdom in Mana and Constitution in Stamina; equipping a greatsword on a Starseer is refused with the shortfall named.
...
Advanced → Stats is the single settings home for HP, Mana, Stamina, Actions
and Draw conversions; hand and draw rules; and Attack Rating, Defence Rating,
Potency Rating, Poise and Ward
formulas, item and relic bonuses, status bonuses and resistance weights,
diminishing-return constants and caps, impact categories, attack overrides and
break penalties. Each conversion and formula topic shows an expanded live
example using the current character when available. A derived trait is shown as
`base + floor(attribute multiplier × stat) + floor(level multiplier × levels after 1)`.
Every rating is `base + floor(global multiplier × Σ floor(weight × attribute))`: each attribute's contribution is floored on its own, and the scaled sum is floored again so a fractional multiplier still yields a whole rating. A weight is the rate that attribute converts at. Default formulas: Attack Rating=floor(0.5×STR); Defence Rating=floor(0.5×DEX); Potency Rating=floor(0.5×WIS)+floor(0.5×INT); Poise=1+floor(1×CON)+floor(0.5×STR); Ward=1+floor(1×WIS)+floor(0.5×INT). Poise and Ward open at a base of 1 because a vessel of nothing is not a vessel. Every formula exposes a base and attribute weights; one global multiplier scales all five. Equipped weapons and armour, relics, mounted properties and active status bonuses contribute additively. Physical weapon Attack Rating contributes Attack Rating; magical weapon Attack Rating contributes Potency Rating; item Defence Rating contributes Defence Rating; body-armour Poise contributes Poise. Additional per-item bonuses are configurable. Temporary status bonuses to Poise or Ward affect resistance, not the current break threshold.
