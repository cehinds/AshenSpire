...
5. **Headless engine.** Nothing under `src/engine/` or `src/model/` references `document`, `window`, `localStorage`, or timers. A combat runs to completion in Node with no UI imports (`tests/combat-rules.test.mjs`, `tests/solo-run.test.mjs`).
...
| `tools/` | Node-run instruments and harnesses. The observed-red idiom (`--selftest` / `--mutate`) lives here; the arms are run by hand when a tool changes. |
| `tests/` | Five scenario tests, run by `node --test "tests/*.test.mjs"` (§8). `ui-screens` and `coop` mount screens on a test-local fake DOM; the rest import model, engine and content only. |
...
- **Acceptance** (`tests/coop.test.mjs`, elite door): per-seat chest offers differ by seat and
...
- A `schemaVersion` **newer** than this build is refused and **preserved**, as the profile is
  ...
  current one, loads and migrates forward; `tests/solo-run.test.mjs` proves the newer-save
  refusal and carries the legacy save fixtures forward through the migration path.
...
*Falsify the set:* the profile-durability cases (no profile, corrupt profile, older version,
**newer** version, two archives in a row); `tests/ui-screens.test.mjs` mounts the newer and
corrupt notices and the archive screen, and the durability cases themselves are not pinned by a
scenario test — and the drawer's own
...
7. Property rules (§3.3 PropertyRule): every `property` tag has exactly one rule; a rule's tag is
   ...
   *cards never carry properties*. Each refusal names its row.
...
3. **Player acts:** play any affordable cards, use flasks, inspect piles. Max hand size is the Hand size row (§3.5; the retired fallback of 5 in a pre-ruleset-7 co-op seat) — excess drawn cards go to discard with a "hand full" toast (StS behavior). An overflow draw takes only what the draw pile holds: a full hand never reshuffles the discard pile, so a card that overflowed is not shuffled back to overflow again (`actions.js drawCards`).
...
- **Hand:** bottom center, fanned, max 10; hover raises card ×1.5 with full text. A hovered or selected card rises **at its fan angle** — it never snaps upright (§7.4.1).
...
- Floating damage/heal/block numbers, born mid-body at 55% of the sprite's height (`FLOAT_ANCHOR_Y`, `src/ui/fx.js`) so they clear an enemy's intent badge; brief target flash on hit that opens on **one white frame** (a white silhouette for the first 10% of the flash, then the red tint); **tiered screen shake on every landed hit**, scaled by residual damage (`shakePx`, `src/ui/fx.js`): ≤2 px below 15 (1 → 2 px), ≤4 px at 15+ (3 → 4 px, capped) — guard-absorbed damage never shakes, and hit-stop stays heavy/crit-only (below). **No animation blocks input, and a click always skips to end-state.** At the default animation speed, most effects run ≤300 ms; queued events play out one `stepMs` apart in paced playback (90 ms at normal, 140 slow, 45 fast — not the 80 ms this line once claimed) and 80 ms apart in the instant / reduced-motion fast-float path (`STEP_MS`) — but a few big-moment effects are hardcoded past that bound (heavy hit flash 380 ms, cast glyph 450 ms, Stagger wobble 600 ms) and the Animation speed setting (slow / normal / fast / instant) scales the *pacing* (beat, step, lunge), never those fixed effect durations. The Screen shake, Reduced motion, and Reduce flashes settings each suppress their effect entirely (`src/ui/fx.js`).
- Bleed burst and Stagger get distinct, slightly bigger effects (they're the theme).
- **Paced-playback pacing** (`src/ui/fx.js` `groupBeats` / `playTimeline`). Paced playback groups a dispatch's log into beats — one per card play, flask or enemy move, plus turn banners — and plays each as wind-up → visuals every `stepMs` → HUD apply (`onBeatApplied`) → breath.
  - **Card-play latency.** A card's price is paid before it is announced (the engine emits `energySpent` / `manaSpent` / `staminaSpent`, then `cardPlayed`). Those payment receipts are **never a beat of their own**: they ride the next actor's beat as its `paid` prelude, and the moment that beat starts they pulse the orb and reach the HUD (`onBeatApplied` with just those receipts; the rest of the beat applies at its end as before). Target: click → first damage number is the attack clip's own impact time with **no pre-roll** — the old actor-less payment beat cost `stepMs + beatMs` (≈490 ms at normal) before the swing even began. At normal speed with the current clips that is 700 ms for `swordShieldAttack` (impact frame 7 × 100 ms; 1190 ms before), and it falls with the clip retime. The **card-play sound** (`cardPlay`) belongs to the same moment: `playTimeline` sounds it as the play's beat starts, with the swing and the orb pulse — no longer at dispatch, where it led the impact by ~1.2 s. Instant speed and Reduced motion have no timeline: everything, cost included, shows at once, and the play's own fast-float step sounds it.
  - **Enemy-turn pacing.** Enemy moves chain with a short breath, `enemyBreathMs` in `ANIM_SPEEDS` (slow 260 / normal 150 / fast 70 ms), instead of the player's `beatMs`. A turn banner beat costs **nothing** when no banner is drawn — solo shows the turn in its permanent `.turn-ribbon`, so it never draws one — and a drawn banner leads by one `stepMs` and plays over the first wind-up. Target at normal speed: an enemy turn (the whole End Turn dispatch, banner to refilled hand) ≤ 0.9 s + 0.45 s per enemy; measured through the real `playTimeline` with a lunge-timed enemy move, 1.22 / 1.63 / 2.04 s for 1 / 2 / 3 enemies (was 2.27 / 2.93 / 3.59 s). A click still skips to end-state, and while a paced enemy turn plays a dim **"Click to skip"** (`combat.skipHint`, `content/source/uiStrings.csv`) sits under the ribbon — `aria-hidden`, never a control, and absent at Instant speed or under Reduced motion, which have nothing to skip.
  - **Ghost HP bar.** When a health bar drops, a pale trail stays at the old value for `lagMs` (260), then drains linearly to the new one over `drainMs` (420) (`GHOST_BAR`, `src/ui/components/resbars.js`). It is drawn under the fill, so only the span just lost shows. Bars are rebuilt every render, so the trail lives per bar key as a timeline and each rebuild draws it as it stands now: a re-render mid-drain carries on, a second hit during the lag holds it at its top, a heal above it clears it. Solo (the player's HUD and every combatant's under-model strip) and co-op (the same component) both key it by entity. **Reduced motion and Instant speed draw no trail** — the bar just jumps.
...
Presentation only: no engine event, run field or save key changes.

- **Attack clips land fast.** Every painted attack clip (`content/config/ui/presentation/equipmentAnimations.json`, `motionProfiles.*.clips`) steps at 60–70 ms, starts on its first wind-up frame (no leading ready frame), holds its impact frame for two steps, and borrows no `DEFEND` / `BUFF` in-betweens. Its impact — `frameMs × impactIndex`, read by `animationTiming()` — lands **≤ 250 ms** after the clip starts at normal speed: greatsword, sword & shield, dagger, unarmed physical and the unarmed magic channel at 240 ms, twin swords at 195 ms (they were 500 / 700 / 500 / 700 / 800 / 480). Hit-stop and the kill cam (§7.4) start from that same `impactMs`, so they meet the impact frame by construction. The art records the importers read (`art/*/attack-sequence.json`, `art/unarmed-*/runtime-fragment.json`) carry the same clips. *Exempt:* the legacy 60-step Reaver strip (`src/ui/reaverAttack.js`, impact 1 736 ms) plays only when the combat figure has no painted outfit, which the shipped Reaver always has; it stays as the fallback, unretimed.
- **Idle breath.** Every living combatant's painting drifts 2.5 px up and back on the standalone `translate` property (`styles/combat.css` `sprite-idle`), so it composes with the facing layer's mirror, the art's own foot-line transform and the hit animations rather than replacing them. It is aimed at the painting inside the figure (`.facing > img`, or a pose stage's `.pose-layer`, whose frame img has its animations cancelled on every pose change). Period and phase are per actor — 2.8–3.6 s and a start anywhere in the cycle, derived from the combatant's id (`assets.js` `idleRhythm`) — so a row never bobs in unison and a re-render keeps the rhythm. Dead and downed figures hold still; Reduced motion (in-game or OS) and lite rendering turn it off; a hit-stop pauses it with everything else.
- **Enemy attack pose.** An enemy with an attack drawing crossfades idle → attack → idle over `--enemy-pose-fade` (100 ms), centred on 25% and 80% of the actor's playback — never a one-frame visibility pop. Its facing layer coils (squash) before the lunge, stretches along it and settles with an eased overshoot, on the standalone `scale` property. Reduced motion drops the squash (the fade stays); instant playback never poses the enemy. The player's painted stages keep their own crossfade (`paintedOutfits.js`).
- **Hover lift.** A hand card's transform is one composed expression, `var(--card-fan-transform) translateY(var(--lift)) scale(var(--lift-scale))` (`hand.js` `CARD_FAN_TRANSFORM`); hover, selection and inspection set only `--lift` / `--lift-scale`, never `transform`, and no state rule uses `!important` on a card's transform. A lifted card keeps its fan angle, and its hit lane undoes the lift so the target under the pointer does not move. In solo combat, while one card is selected, hovering another does not lift it.
...
**One runner, five scenario tests.** The suite is `node --test "tests/*.test.mjs"` — Node's
built-in runner, no custom harness, no registration list (a new `tests/*.test.mjs` is picked up
by the glob; quote it, because Node 22's `--test` does not take a bare directory). Each file is
**one** `test()` that drives the real game through a whole scenario and checks invariants at
every step, rather than many narrow cases:

| file | what it drives | what a red means |
|---|---|---|
| `tests/solo-run.test.mjs` | every class climbs 16 seeded runs toward act 3 (the `tools/runsim.mjs` pilot inside the `main.js` run loop), through the real save door at every node, reward row and one turn of every fight; each seed is replayed without reloads and must match step for step; the legacy save fixtures migrate and play on | a run-flow rule (rewards, pity, chests, boss relic choice, level/XP, flasks, graces, events, shops, seats and act maps) broke, a save round trip lost state, or a seed stopped replaying |
| `tests/combat-rules.test.mjs` | one table of rows `{ name, setup, actions, expect }`, each a fresh fight built by the game's own factories | an exact combat number moved: damage, Block, statuses, poise, hand rules, attack sources, Weapon Art charge, juice gates |
| `tests/coop.test.mjs` | one seeded party on the real host (`tools/session.mjs`) with every intent through the LAN door, seats dropping, joining and rejoining, a disk save → restore, and the co-op screen mounted on a fake DOM | co-op flow, per-seat rewards or catch-up, reconnect replay, or co-op/solo parity broke |
| `tests/content.test.mjs` | `validateContent` on the shipped bundle, the drift and reach tools (`content-build --check`, `config-build --check`, `attack-source-audit --check`, `linkcheck`, `statusreach`, `closedsets`), ui-string ids, asset paths, and malformed bundles refused by name | shipped data is invalid, a generated file is stale, a link or asset is missing, or a validation door has stopped refusing |
| `tests/ui-screens.test.mjs` | every screen in `src/ui/screens` mounted from a real game state on a test-local fake DOM | a screen throws, shows a raw string key or unfilled `{token}`, draws an unwired button, or ignores reduced motion |

A new rule extends the scenario that already covers it — numbers in `combat-rules`, run flow in
`solo-run` or `coop`, data in `content`, screens in `ui-screens` — instead of adding a file.
The index is derived, never hand-kept:

```
node --test "tests/*.test.mjs"                # the whole suite
grep -n "^  { name: '" tests/combat-rules.test.mjs  # the combat table's rows
```

**The required-coverage list that used to live here is gone, and deliberately.** It named 17
tests against a suite that had already grown past it, and two of its entries had gone false
without anybody editing them — test 7 restated Bleed's threshold as `12` with an escalating
`×1.5` (both superseded by the constant-threshold proc vocabulary, §4.4) and test 8 named
"Scarlet Rot", renamed at the IP scrub. **A hand-kept index of tests is a cache of `grep`**, and
this one rotted exactly the way §3.2's file inventory did.

**What this section states instead is the contract a test must meet, which cannot rot:**

1. **Headless.** Every test runs in Node with no browser. `ui-screens` and `coop` mount screens
   on a test-local fake DOM; the other three import only model, engine and content. A check
   that needs a real browser belongs in `tools/`, not here.
...
4. **A check is trusted only after it has been watched to fail.** The observed-red idiom is
   ...
   (reinstate the defect N ways, each must be caught), carried by the `tools/` instruments. CI
   no longer runs those arms; run a tool's arm by hand when you change that tool
   (DEVELOPER.md, *Plants and the same door*). In the suite, `content.test.mjs` refuses
   malformed bundles by name for the same reason.
...
is called done." CI exists — `.github/workflows/tests.yml` runs the suite. What survives is the
standard: all green before a milestone is called done, on whichever machine you ran it.)*
...
Acceptance (`tests/combat-rules.test.mjs`, the Art meter rows): accrual per qualifying hit and not from the Art itself; the cap; stagger/burst credit; the unleashed play adds its effects and spends the meter while a non-full play spends nothing; per-weapon values survive a swap out and back; a snapshot round-trips the meter and a snapshot without the field loads at 0; content validation refuses a malformed unleashed form. Screenshots of the combat HUD at a partial and a full meter live in `docs/evidence/game-feel-rework/weapon-art-charge/`. Co-op (`tests/coop.test.mjs`): each seat's meter fills from its own weapon hits only, a full seat's Art unleashes and empties only that seat's meter, and the host snapshot and the co-op board carry both; screenshots in `docs/evidence/game-feel-rework/coop-parity/`.
...
*Falsify:* every shipped class is free; a fixture `class` row gates the Rogue until earned; `classLevel 3` is not met at 2 and met at 3, `bossWithGroup` needs the boss AND the group, the tally only grows; a Reaver at class level 1 with Iron Footing picked swaps to Rogue: `class` and `zones.core` read rogue, the pick is dropped, `class:reaver` is gone, the blade skill, the deck and the relics stay, a `classSwapped` row is written, the save is sound and round-trips; the same class is a no-op, an unknown class refused by name; the opcode lands a named class through the run-effect door and a random swap never lands on the run's own; the mirror ships with the opcode and durable choice ids; a swap to an unknown class is refused by name at validation; a locked class card wears its hint.
...
*Falsify:* a Mana card with no stamina line, no action line, or an upgrade that drops the action line under a Mana cost is refused by name, an X-cost Mana card passes, a negative floor and a missing `balance.mana` are refused, a stagger status the bundle lacks is refused, a Mana spell building 1 is refused, `arcaneBuildup` with neither or both selectors is refused; a self-aimed pour of the player's whole max applies 2 Vulnerable and 2 Weak, emits one `playerStaggered` naming one action, grows the meter ×1.25, opens the next turn one action short and the turn after whole; the receipt is Constitution × the row + body armour + relics and `createCombat` stamps it; a starseer's Ash Staff break batters the foe's Poise by 6 and refunds nothing, a Goldbough Branch break pours half the threshold into the other foe and none into the broken one (property-mount tests).
...
*Falsify:* the inn's set includes `questBoard` and the shrine's, chapel's and camp's do not; every town posting a quest opens its board at its inn; a town lists its quest `open` with its reward and speaker; accepting through the exchange moves it to `accepted` and completes nothing, a collect before the objective is refused by name, the report is spoken on return, and the collect pays `rewardCinders` once, writes one `questCompleted` row with `source: 'atlas'` and shows the quest `done` and in the journal; a second collect before or after a reload is refused and pays nothing; Leave changes nothing and an unknown response is refused by name; a Nameless step in history lists the chain as started and its completion moves it to completed (`tests/ui-screens.test.mjs` mounts the board from a real run; the collect and refusal steps are not pinned by a scenario test).
...
- **What a grade prices.** Only the ATTACK role's card — the strike the graded piece throws, rated `ar` or `pr` as its attack profile's `ratingId` names — in `effectiveEquipmentRating`, the one door every card amount goes through (`roleAmountReceipt` hands the run's table in for the attack role only). The same piece's guard and technique cards keep the flat rating even where they read `pr`. For a graded piece each attribute's term becomes
  ...
  where `w` is the rating row's own weight for that attribute (the stat row, §3.5) and `c` is the grade's coefficient floored at `w` — `max(grade coefficient, w)`, so a low grade on a heavily weighted row pays the flat rate rather than less; an attribute the weapon has no grade for keeps the flat term `floor(w × v)` exactly, and `anchor` is `balance.weaponScaling.anchor` (**3**, the lean scale's class stat). The terms are summed and multiplied as before, and the rule's `base` is added. So at or below the anchor a graded weapon reads EXACTLY the flat rating (a stock character's creation damage does not move), and every point above it is paid at the grade: +2 per point on S, +2/+1 alternating on A, +1 on B. Because `c` is floored at the row weight, a graded weapon's term is never below the flat term at any attribute value. `dr`, Poise, Ward, the opt-in `balance.combatRatings` module's attribute receipt and every ungraded piece keep today's formula byte for byte — and while that module is switched on (`balance.combatRatings.enabled`) the fight prices a strike off the player's own ratings, so the attack stamp, the Level-up preview and the shrine's grade fact read flat there too, never promising a grade the fight does not pay.
...
What does change for a **new** run on an existing seed: which seat the run opens in. That is the feature, and it is the one thing this section is allowed to change about a seed's replay. `tools/runsim.mjs` pins claims 1–3, and `tests/solo-run.test.mjs` carries claim 2's legacy saves through migration and plays them on; the pre-seat map fingerprint (claim 1) and claim 4 are no longer pinned by a scenario test.
