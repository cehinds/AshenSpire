...
- **Combat scenery**: `regionForRun(run)` returns the seat's region — no seed hash, no rotation. `combatEnvironment` is otherwise unchanged (road / city / dungeon settings, saved scene selection). The `ashen-crown` region is **not** a seat: under this section it is the scenery of the **boss node of the tier-3 act**, whichever seat holds it (LORE §1: "the Ashen Crown is not a place"). `drowned-coast` stays a World Journey region only.
...
### 13.5 The last seat hosts the Ashen Crown
