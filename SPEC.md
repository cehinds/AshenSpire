...
9. **Presentation.** The player's combat frame shows one pip bar per equipped weapon with a meter (weapon name, `value/max` pips, accessible label). A meter that has just become full flashes once; with reduced motion (OS preference or the in-game setting) it shows a static highlight instead. The Art card in hand carries the same pips and, when full, an *Unleashed* marker naming the extra effects. The engine stays headless; the UI reads `combat.artCharge` through `src/model/artCharge.js`.
