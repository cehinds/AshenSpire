...
|---|---|---|
| Maximum HP | `36 + floor(2 × CON) + floor(2.5 × (level − 1)) + flat bonuses` | Derived-stat ruleset 7 (§13.4m). Flat bonuses are the declared relic/equipment/run folds, not class-id branches. |
| Opening hand / draw | `5 + floor(0.2 × INT) + floor(0.1 × (level − 1))` | Ruleset 8 (§13.4l, plan A4). One value owns both the opening hand and cards drawn per turn (§4.1). |
| Defense | `-6 + DEX` | The data-owned base value used by the basic guard/defense profile. |
| Actions / turn | `3 + floor(0.25 × DEX)` | Ruleset 7: one more Action at DEX 4 and at DEX 8. The player-facing Actions value; engine naming may migrate separately. |
...
the `draw` row of derived-stat ruleset 8: `5 + floor(INT × 0.2)` plus its level
...
whose derived-stat snapshot predates ruleset 8 (a base-3 Draw row) keeps the
...
2. **Player turn start:** lose all block (unless modified), set energy to 3 (base), draw the Draw stat (5 for a new character since ruleset 8), `playerTurnStart` triggers.
...
- **Derived-stat ruleset 8 (plan A4, owner ruling 2026-09-24).** Ruleset 6's one format (`base + Σ floor(weight × attribute) + floor((level − 1) × perLevel)`) with ruleset 7's rows (§13.4m, plan A3's Actions and HP) and one more row moved: `draw = 5 + floor(INT × 0.2)` plus its 0.1 per level, where rulesets 6 and 7 said 3. Solo combat now deals this row as the hand every turn (§4.1), and at creation (INT 1–4) the old base dealt three cards. Rulesets 1–7 stay readable and a run keeps the snapshot it was born with — a ruleset-6 or ruleset-7 run keeps drawing 3, and keeps the retain-and-fill hand defaults its Draw row was priced for (§4.1).
...
- **The pools are the stats; there is no conversion scale** (owner, 2026-09-21, superseding this bullet as first written). `lean` shipped with `statConversionScale: 1/5`, which divided every rule's tier by five so one lean point read as one tuned2 tier. It was removed with derived-stat ruleset 6: *"all calculations should be sum(floor(statmult*stat)) + equipment bonus"*. Every derived row now reads the attribute the character sheet shows, `base + Σ floor(weight × attribute) + floor(perLevel × (level − 1))`, and no creation mode converts an attribute on its way in. A run snapshotted under the scale restores its own rows.
- **Derived-stat ruleset 7 retunes the rows for the lean scale** (plan A3; ruleset 8, §13.4l, then moves only the Draw base, 2026-09-24, measured with `tools/runsim.mjs`). `energy = 3 + floor(0.25 × DEX)`: the first extra Action sits at DEX 4, which creation can reach, and the next at DEX 8, inside a climb's levels (it was DEX 5 and 10 under ruleset 6, where 5 was past the creation ceiling). `hp = 36 + floor(2 × CON) + floor(2.5 × (level − 1))`: half of Constitution's HP moves onto the level, so Constitution — which also pays Stamina and Poise — stops being the only level-up pick that grows the pool. A stock Reaver opens on 50 HP (the Forsaken Medallion's ten included), Rogue and Herald on 40, Starseer on 52 (38 plus its Starstone Shard's fourteen, plan A2; re-measured on ruleset 7, a ten-point Shard drops the Starseer to 37/120 wins, below the band). `draw`, `stamina = 1 + CON`, `mana = 1 + WIS` (+ ½ INT) and `poise = 1 + CON` are unchanged. The simulator's per-encounter damage shows act 1 ending few runs and the act-2 boss ending most, so the level's growth is what was retuned, not #1238's 70-HP opening. Rulesets 1 through 6 remain readable, and a run keeps the ruleset it was born under.
- **The Dodge Roll reads the lean scale** (plan A3). Its Dexterity term is `floor((DEX − 3) / 2)` (`mechanics.dodgeRoll.dexterityCentre`, `dexterityPerModifier`), not the d20 habit `floor((DEX − 10) / 2)`, which was −4 or −5 for every creatable character. A landed dodge's guard is `max(1, 3 + DEX term + class guard)` (`temporaryGuardMinimum`). The Weight Class prices the dodge at Light 1 Stamina / 0 Actions, Medium 1 / 1, Heavy 2 / 1 — Heavy's former 3 Stamina was more than a Constitution-1 pool.
