...
- **What a grade prices.** Only the ATTACK rating of the graded piece — `ar` or `pr`, whichever its attack profile's `ratingId` names — in `effectiveEquipmentRating`, the one door every card amount goes through (`roleAmountReceipt`). For a graded piece each attribute's term becomes
...
- **Where the grades are shown.** The equipment card's requirement line gains `Scales STR S · DEX D` for a graded weapon (the tooltip says what a grade pays). The shrine's Level-up rows lead with the weapon facts for their attribute: `STR A — +2 dmg next point on Greatsword`, the number being the equipped piece's own attack receipt at `v + 1` minus at `v` (the pending value), never a coefficient restated.
