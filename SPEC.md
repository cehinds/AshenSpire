...
Enemy card movesets are a limited presentation and authoring extension over the existing deterministic authored move plans established by #239/#241. Preserve phase transitions, repeat limits, delayed actions, and other existing plan constraints, plus the saved plan cursor and current intent. Do not introduce a separate parallel move picker or reroll an intent when rendering a card or loading a save.
