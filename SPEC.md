...
- **The schools are derived, no second table** (`skills.js skillSchools`): a weapon or focus track drafts from the card-domain tags its held pieces of that item type carry in `tagging.csv` (a straight sword: `blade`, `basic`; a greatsword: `blade`, `heavy`), falling back to every piece of the type when none is held; `dualWield` reads both hands; armour and class tracks have no schools (nothing to draft until 5b's tree). "The sword you levelled drafts sword cards."
- **The roll** (`engine/encounters.js rollSkillDraftIds`): the class reward pool filtered to the track's schools and to the rarities the level has opened — `balance.skill.rarityUnlock { common: 1, uncommon: 4, rare: 7 }` (the game has no legendary rarity, so the proposal's fourth row has no seat) — weighted by `rewards.rarityWeights.normal`, `draftSize` distinct picks on the `cardRewards` stream. An empty pool draws nothing.
- **Rows are keyed** (`rewardplan.js rowKey`): a row's state lives under `states[row.key]` — the kind for the kinds an offer carries once, `skillDraft:<skillId>` for a draft — so two drafts never share a state and a pre-draft save's `states` (keyed by kind) still read. `resolveContinue` resolves a draft as it resolves the card offer (the injected pick); `rewardClaimStatus.requiredChoice` names the first choice waiting, draft before card, with its key. Taking a draft (`reward.js apply.skillDraft`) pushes the card — upgraded when the track has reached `upgradeAt` — spends the queued draft (`spendSkillDraft`, the door's one write to the ledger) and records the pick in `pendingReward.chosenDraftCardIds[skillId]`; `validateRunShape` refuses a draft state key the offer does not carry, a chosen card not of its draft, and a Taken draft without its card, each by name.
- **The threshold upgrades the deck** (`awardSkillXp` → `applySkillUpgrades`): the award that carries a track across `balance.skill.upgradeAt` sets `upgraded: true` on every deck card of the track's schools, once, and reports the ids; a draft taken at or above the threshold arrives upgraded; the shrine keeps the untagged cards.
