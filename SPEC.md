...
- **Shared Presentation primitives.** The public primitive Component Model ids are `panel`,
  ...
  `item-slot`, `folding-tray`, `tray-header`, `tray-content`, and `tooltip`. Specialized
...
- **Menu and Armoury Component Models.** The contextual launcher is `quick-menu-panel`,
  ...
  shared `folding-tray` → `tray-header` + `tray-content` composition. The `trayModel` factory owns edge,
  ...
  anchored edge, including the open Right Tray form `> TRAY NAME`. These ids describe the existing data-driven Quick Menu and Armoury
