...
- **Derived-stat ruleset 5.** `hp = 20 + 4 × CON`, `mana = WIS`, `stamina = CON`, `energy = 2 + floor(DEX / 5)`, `draw = 4 + floor(INT / 5)`, plus the phase-6 level thresholds each row already carried. Mana and Stamina drop their five-point tier: a point of Wisdom IS a point of Mana and a point of Constitution IS a point of Stamina, which is what makes a signature art costing two of each playable on the first floor — the deviation §13.4k deferred to this phase. Rulesets 1 through 4 remain readable: a run restores the snapshot it was written with, and the required row set is a function of the version, so a version-4 save is never asked for a row it never had.
...
- **Equipment minima gate the mutation, not a screen.** `equipmentRequirements.csv` is rebased onto the 3–12 scale, and `canEquip` refuses a named item the run's attributes cannot hold, naming the shortfall — the same door the grip refusal uses, so a drop, a drag, a gamepad path or a second surface cannot walk past it. Asked without an item or without attributes, the question keeps its old answer.
