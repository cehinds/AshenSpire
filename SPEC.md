...
- **Validation, by name:** a class carrying `favored` with no item-type tag (`tagging.class.<id>`); `favoredXpMult` below 1; a dangling `abilityCard` or `kitRelic` (schema refs); `roleCopies.ability` other than 1 when present.
