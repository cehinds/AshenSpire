...
async function settle(page) {
  ...
  await page.waitForFunction(() => {
    ...
    const feet = [...document.querySelectorAll('.combatant[data-formation-depth="0"] .sprite')].map(s => s.getBoundingClientRect().bottom);
...
async function check(page) {
  const result = await page.evaluate(() => {
    ...
    const feet = sprites.filter(s => s.closest('.combatant').dataset.formationDepth === '0').map(s => s.getBoundingClientRect().bottom);
    const images = [...document.querySelectorAll('.enemy-pose-idle,.painted-stage .pose-frame')].filter(i => getComputedStyle(i).display !== 'none' && i.closest('.combatant').dataset.formationDepth === '0');
