...
//
// The clean edge went red after hover placement deliberately stopped following
// the pointer (b0dc912/d705b66): the tooltip now sits beside the element it
// explains and clears its sibling group. The old assertion still measured the
// distance to the cursor inside that element, so it called the intentional
// element-to-panel separation a regression. The browser door stays the same;
// only the asserted product contract follows the later accepted behaviour.
...
const VIEWPORTS = [
  { w: 800, h: GATE_BELOW_H }, // smallest admitted landscape viewport; derived from the upright gate
  { w: 1024, h: 640 },  // 0.85
...
const TIP = (anchor, siblings) => `(() => {
  ...
  // One separating axis is exactly --place-gap; the free axis may slide to
  // remain on screen, so box-to-box distance is the stable quantity.
...
async function main() {
  ...
  for (const vp of VIEWPORTS) {
    ...
    await wait(80);
    // Hover placement is stable when the pointer moves inside the same element;
    // the anchor is the card box, not the last pointer coordinate.
    // Hover lifts the card, so re-read the accepted anchor AFTER the transition;
    // the pre-hover rect is no longer the element attachTooltip placed beside.
