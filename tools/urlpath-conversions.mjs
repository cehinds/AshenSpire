...
function stripComments(source) {
  ...
  for (let i = 0; i < source.length; i++) {
    ...
    if (mode === 'string') {
      out += c;
      if (c === '\\') { if (i + 1 < source.length) out += source[++i]; continue; }
...
function selftest() {
  ...
  say(clean.findings.length === 0, 'clean tree has no hand-rolled conversion', clean.findings.map((f) => `${f.path}:${f.line}`).join(', '));
