...
if (args.includes('--selftest')) {
  ...
  if (wantedPlant && sourcePlants.length === 0 && geometryPlants.length === 0 && artifactPlants.length === 0) {
...
const EVIDENCE_READING = `(() => {
  ...
  const accessibleName = (node) => {
    ...
    const labelled = labelledBy
      ...
      : '';
    return (node.getAttribute('aria-label') || labelled || node.getAttribute('title') || node.textContent || '')
      .replace(/\\s+/g, ' ').trim();
  };
  const describe = (node) => {
...
async function main() {
  ...
  try {
    ...
    if (screenshots) {
      ...
      writeFileSync(join(ROOT, 'docs', 'preview', manifestName), `${JSON.stringify(manifest, null, 2)}\n`);
    }
