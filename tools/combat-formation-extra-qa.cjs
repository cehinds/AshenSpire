...
await p.goto(base+'?shot=atlas',{timeout:60000});await p.waitForSelector('.atlas-world');await p.waitForTimeout(500);const zoom=await p.locator('.atlas-world').evaluate(e=>getComputedStyle(e).getPropertyValue('--atlas-zoom'));if(+zoom!==2.5)throw Error('Wrong initial map zoom');await p.locator('[data-atlas-zoom="0"]').click();if(+(await p.locator('.atlas-world').evaluate(e=>getComputedStyle(e).getPropertyValue('--atlas-zoom')))!==1)throw Error('Fit failed');await p.locator('[data-atlas-center]').click();await p.screenshot({path:'outputs/combat-formation/atlas-focused-phone.png'});
if(errors.length)throw Error(errors.join(';'));fs.writeFileSync('outputs/combat-formation/additional-checks.json',JSON.stringify({passed:['Six enemy nameplates can be pointed at','Defeat retains formation','Co-op keyboard and pointer blocked during enemy playback','Atlas starts at 2.5x; Fit and You work'],errors},null,2));console.log('PASS six enemies, defeated slot, input lock, atlas camera');await b.close();})().catch(e=>{console.error(e);process.exit(1)});
