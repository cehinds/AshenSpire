...
// The tree's variable bindings name balance rows; the framework derivation
// resolves them at build (deriveFrameworkFromTree says why), so the one
// authored tuning home is read here too.
const { balance: BALANCE } = await import('../src/content/balance.js');
function balanceAt(path) {
  let node = BALANCE;
...
function compileDir(srcDir, outDir, { write } = {}) {
  ...
  for (const file of readdirSortedSync(srcDir)) {
    ...
    stale += place(outDir, name, emit(name, data, file), write, produced);
    results.push({ file, name, rows: Array.isArray(data) ? data.length : Object.keys(data).length });
  }
  // THE TREE'S DERIVATIONS. Seven modules used to have sources of their own
  // (tags/tagDomains/tagFamilyDomains/propertyRules/propertyRuleEffects under
  // content/source, properties/relations under content/framework). They are
  // views of the node tables now, emitted here so every reader of the old
  // shape keeps reading it while the authored home is one tree.
  if (parsed.has('nodes')) {
    for (const [name, data, source] of deriveFromTree(parsed)) {
      stale += place(outDir, name, emit(name, data, source), write, produced);
      results.push({ file: `(${source})`, name, rows: Array.isArray(data) ? data.length : Object.keys(data).length });
    }
    for (const [name, data, source] of deriveFrameworkFromTree(parsed)) {
      const fwOut = join(outDir, '..', '..', 'framework', 'data');
      if (write) mkdirSync(fwOut, { recursive: true });
      stale += place(fwOut, name, emitFramework(name, data, source), write, new Set());
      results.push({ file: `(${source})`, name: `../../framework/data/${name}`, rows: Object.keys(data).length });
    }
  }
  const orphans = existsSync(outDir)
    ? readdirSortedSync(outDir).filter((f) => f.endsWith('.js') && !produced.has(f))
    : [];
  return { results, stale, orphans, parsed };
}

/** Write one generated module if it changed; 1 when stale, 0 when current. */
function place(outDir, name, code, write, produced) {
  ...
  if (comparable === code) return 0;
  if (write) writeFileSync(dest, code);
  return 1;
}

function emitFramework(name, data, sourceFile) {
  return (
    `// GENERATED by tools/content-build.mjs from content/source/${sourceFile}\n` +
    `// Do not edit by hand — edit the source file and re-run the build.\n\n` +
    `export const ${name} = ${JSON.stringify(data, null, 2)};\n`
...
/** parentId chain from a node up to its root, root last. */
function rootOf(byId, id) {
  let node = byId.get(id);
  const seen = new Set();
  while (node && node.parentId) {
    if (seen.has(node.id)) bthrow(`nodes.csv: '${id}' sits on a parent cycle`);
    seen.add(node.id);
...
/**
 * deriveFromTree(parsed) → [[name, data, source], …] for content/generated.
 *
 * tags        every non-root node, domain = its root's id
 * tagDomains  every root
 * tagFamilyDomains  familyNodes rows, nodeId lifted to its root — the family
 *             may carry the ROOT's whole domain in the old model, so a row
 *             naming a branch widens to the root here and validate.js keeps
 *             the narrower branch rule itself
 * propertyRules / propertyRuleEffects  the conferring nodes — every node with
 *             an effects entry — with requires/excludes read off the relations
 *             and every {variable} replaced by the balance path its default
 *             binding names, which is the shape src/content/propertyRules.js
 *             resolves at load exactly as before
 */
function deriveFromTree(parsed) {
  const nodes = parsed.get('nodes') || [];
  const relations = parsed.get('nodeRelations') || [];
  const familyNodes = parsed.get('familyNodes') || [];
  const terms = parsed.get('nodeTerms') || [];
  const bindings = parsed.get('variableBindings') || [];
  const effects = parsed.get('nodeEffects') || {};
  const byId = new Map(nodes.map((n) => [n.id, n]));
  const str = (v) => (v == null ? '' : String(v));

  const tagDomains = nodes.filter((n) => !n.parentId).map((n) => ({ id: n.id, label: str(n.label), aside: n.aside === true || n.aside === 'true', blurb: str(n.blurb) }));
  const tags = nodes.filter((n) => n.parentId).map((n) => {
    const root = rootOf(byId, n.id);
    if (!root) bthrow(`nodes.csv: '${n.id}' names parent '${n.parentId}', which is not a node`);
    // `visibility` rides along so model/tags.js can tell a chip from a
    // framework node: a node with a visibility is shown through the framework's
    // terms, never as a chip, so it owes no colour or glyph.
    return { id: n.id, domain: root.id, label: str(n.label), color: str(n.color), glyph: str(n.glyph), visibility: str(n.visibility), blurb: str(n.blurb) };
  });
  const seenPair = new Set();
  const tagFamilyDomains = [];
  for (const row of familyNodes) {
    const root = rootOf(byId, row.nodeId);
    if (!root) bthrow(`familyNodes.csv: '${row.family}' names node '${row.nodeId}', which is not a node`);
    const key = `${row.family}\u0000${root.id}`;
    if (seenPair.has(key)) continue;
    seenPair.add(key);
    tagFamilyDomains.push({ family: row.family, domain: root.id });
  }

  const templateOf = new Map(terms.filter((t) => str(t.template)).map((t) => [t.nodeId, str(t.template)]));
  const bindingOf = new Map(bindings.filter((b) => b.scope === 'default').map((b) => [`${b.nodeId}\u0000${b.variable}`, str(b.balancePath)]));
  const substitute = (nodeId, v) => {
    if (Array.isArray(v)) return v.map((x) => substitute(nodeId, x));
    if (v && typeof v === 'object') {
      if (Object.keys(v).length === 1 && typeof v.variable === 'string') {
        const path = bindingOf.get(`${nodeId}\u0000${v.variable}`);
        if (!path) bthrow(`nodeEffects.json: '${nodeId}' reads variable '${v.variable}', which has no default binding in variableBindings.csv`);
        return { balance: path };
      }
      return Object.fromEntries(Object.entries(v).map(([k, x]) => [k, substitute(nodeId, x)]));
    }
    return v;
  };
  ...
  return [
    ...
    ['tags', tags, 'nodes.csv'],
    ['tagFamilyDomains', tagFamilyDomains, 'familyNodes.csv'],
    ['propertyRules', propertyRules, 'nodes.csv + nodeRelations.csv + nodeTerms.csv'],
    ['propertyRuleEffects', propertyRuleEffects, 'nodeEffects.json + variableBindings.csv'],
  ];
}

/**
 * deriveFrameworkFromTree(parsed) → the two src/framework/data modules the
 * framework bridge builds its PropertyRegistry from. A node belongs to the
 * framework iff it carries a visibility; its domain is its root's `domain`.
 */
function deriveFrameworkFromTree(parsed) {
  const nodes = parsed.get('nodes') || [];
  const relations = parsed.get('nodeRelations') || [];
  const terms = parsed.get('nodeTerms') || [];
  const bindings = parsed.get('variableBindings') || [];
  const byId = new Map(nodes.map((n) => [n.id, n]));
  const termOf = new Map(terms.map((t) => [t.nodeId, t]));
  // A framework node's defaultParameters are its default-scope bindings,
  // RESOLVED HERE: the PropertyRegistry spreads them under every instance as
  // numbers, so the generated module carries the number and --check goes stale
  // when the balance row moves. The node itself names only the variable.
  const defaults = new Map();
  for (const b of bindings) {
    if (b.scope !== 'default' || !byId.has(b.nodeId) || !byId.get(b.nodeId).visibility) continue;
    const value = balanceAt(String(b.balancePath));
    if (!Number.isFinite(value)) bthrow(`variableBindings.csv: '${b.nodeId}.${b.variable}' names balance row '${b.balancePath}', which is not a number in src/content/balance.js`);
    if (!defaults.has(b.nodeId)) defaults.set(b.nodeId, {});
    defaults.get(b.nodeId)[String(b.variable)] = value;
  }
  const properties = [];
  for (const n of nodes) {
    if (!n.visibility) continue;
    const root = rootOf(byId, n.id);
    if (!root || !root.domain) bthrow(`nodes.csv: '${n.id}' carries a visibility but its root '${root && root.id}' names no framework domain`);
    const row = { id: n.id };
    if (n.parentId) row.parentId = n.parentId;
    row.domain = String(root.domain);
    row.visibility = String(n.visibility);
    const t = termOf.get(n.id);
    if (t && t.playerTermId) row.playerTermId = String(t.playerTermId);
    if (t && t.tooltipTermId) row.tooltipTermId = String(t.tooltipTermId);
    row.priority = Number(n.priority);
    if (defaults.has(n.id)) row.defaultParameters = defaults.get(n.id);
    properties.push(row);
  }
  const frameworkIds = new Set(properties.map((p) => p.id));
  const rels = relations
    .filter((r) => frameworkIds.has(r.sourceId) && frameworkIds.has(r.targetId))
    .map((r) => ({ sourcePropertyId: r.sourceId, relation: r.relation, targetPropertyId: r.targetId, precedence: Number(r.precedence) }));
  return [
...
// owns balance and equipment, and — since the tag tree gave every object a
// kind row — events, flasks and unlocks too: with rows in tagging.csv depending
// on each of those collections, model/tags.js refuses the missing source BY
// NAME (`tagFamilies.event.source: source 'events' is not a collection…`), so
// the door is named inside validateContent before this tool's guard is reached.
// version, mapConfigs and sfx still validate GREEN when absent and remain
// guarded here. K15 plants every missing key and proves the expected owner
// causally, so a responsibility change is loud and must update this matrix
// rather than silently weakening either layer — which is what this edit is.
const BUNDLE_DOOR_MATRIX = [
  { key: 'version', owner: 'content-build' },
  { key: 'events', owner: 'validateContent' },
  { key: 'flasks', owner: 'validateContent' },
  { key: 'mapConfigs', owner: 'content-build' },
  { key: 'sfx', owner: 'content-build' },
  { key: 'unlocks', owner: 'validateContent' },
...
// file, no code. Since the tag tree, an object is its row PLUS its kind row in
// tagging.csv (every object states what it is; model/tree.js refuses one that
// does not), so the probe plants both. If either needs anything beyond these
// literals, clause 1 of Law 1 has regressed and this smoke should say so.
const PROBE_CARD = {
...
BOUNDARY — what this green does NOT cover (SOP 3, CI expectation 4):
  ...
  - three bundle doors are held outside validate.js by this tool (version,
    mapConfigs, sfx); validate.js owns balance, equipment, and — through the
    tag family source check, now that every object has a kind row — events,
    flasks and unlocks. K15's eight-row matrix plants every missing key and
    proves its expected fail-closed owner. A responsibility change must update
    the matrix; neither layer is silently weakened.
...
async function selftest() {
  ...
  try {
    ...
    const b2 = {
      ...b,
      cards: [...b.cards, PROBE_CARD],
      enemies: [...b.enemies, PROBE_ENEMY],
      tagging: [...b.tagging,
        { family: 'card', scope: '', objectId: PROBE_CARD.id, tagId: `classification.${PROBE_CARD.type}` },
        { family: 'enemy', scope: '', objectId: PROBE_ENEMY.id, tagId: 'classification.enemy' }],
    };
    const v2 = G.validateContent(b2);
    ok(v2.ok, `probe card + probe enemy enter by table row plus kind row and validate (${v2.ok ? 0 : v2.errors.length} errors)`);
    ...
    {
      // K15 — eight causal missing-key plants. Three must pass through
      // validateContent and be caught by this tool; the other five must be
      ...
      for (const [index, row] of BUNDLE_DOOR_MATRIX.entries()) {
        ...
        // "Named" means the refusal names the missing door: as the path it is
        // filed under (`balance.…`, `equipment:`) or, for a collection some
        // other table depends on, quoted in the message — model/tags.js files
      ...
      ok(
        passed === 8 && external.length === 3 && externalPassed === 3 && internal.length === 5 && internalPassed === 5 && cleanDoors.length === 0,
        `K15 door-owner matrix totals: plants ${passed}/8; external ${externalPassed}/3; internal ${internalPassed}/5; clean ${cleanDoors.length === 0 ? 1 : 0}/1`,
      );
    }
    {
