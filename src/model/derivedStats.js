 ...
 */
export function deriveStatIncrease(row, { attributes, classDef, statId = 'derivedStat', graded = null } = {}) {
  ...
  // A GRADED WEAPON'S ATTACK ROW (SPEC §13.4o): `graded` = { anchor,
  // coefficients: { attribute: c } }. A graded attribute's term is
  // floor(w × min(v, anchor) + c × max(0, v − anchor)), counted even where the
  // row itself weighs it 0; every other term is the row's own.
  const gradedIds = graded && plainObject(graded.coefficients) ? Object.keys(graded.coefficients) : [];
  const counts = [...weights, ...gradedIds.filter((id) => !weights.some(([w]) => w === id)).map((id) => [id, 0])];
  const terms = {};
  let points = 0;
  for (const [id, weight] of counts) {
    ...
    terms[id] = gradedIds.includes(id)
      ? Math.floor(Math.min(counted, graded.anchor) * weight + Math.max(0, counted - graded.anchor) * graded.coefficients[id] + EPSILON)
      : Math.floor(counted * weight + EPSILON);
    points += terms[id];
  }
 ...
 * refuses one by name.
 */
export function statRowValue(row, { attributes, classDef, level = undefined, statId = 'derivedStat', lenientAttributes = false, graded = null } = {}) {
  if (!plainObject(row)) throw new Error(`Derived stat '${statId}' has no row`);
  const readIds = [...ruleWeights(row).map(([id]) => id), ...(graded && plainObject(graded.coefficients) ? Object.keys(graded.coefficients) : [])];
  const read = lenientAttributes
    ? Object.fromEntries(readIds.map((id) => [id, Number.isFinite(Number(attributes && attributes[id])) ? Number(attributes[id]) : 0]))
    : attributes;
  const increase = deriveStatIncrease(row, { attributes: read, classDef, statId, graded });
