...
const BALANCE_PRESENTATION = Object.freeze({
  'mana.minActionCost': {
    label: 'Mana — minimum action cost',
  ...
  'mana.minStaminaCost': {
    label: 'Mana — minimum stamina cost',
    settingSection: 'Mana card requirements',
...
function leafRows(value, path = [], rows = []) {
  if (typeof value === 'number' || typeof value === 'boolean') {
    ...
    const retired = RETIRED_BALANCE_PATHS.has(joined) || joined.startsWith('poise.');
    ...
    rows.push({
      ...
      note: presentation.note || (Object.hasOwn(BALANCE_NOTES, joined) && BALANCE_NOTES[joined]) || 'Applies to a new run.',
