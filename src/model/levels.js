...
export function levelScalingReceipt(spec) {
  ...
  let unrounded;
  if (Number.isSafeInteger(base) && Number.isSafeInteger(perLevel)) {
    const exact = BigInt(base) + BigInt(levelDelta) * BigInt(perLevel);
    const safeMin = BigInt(Number.MIN_SAFE_INTEGER);
    const safeMax = BigInt(Number.MAX_SAFE_INTEGER);
    if (exact < safeMin || exact > safeMax) {
      throw new Error('level scaling result: exceeds the safe arithmetic range');
    }
    unrounded = Number(exact);
  } else {
    const scaledDelta = levelDelta * perLevel;
    if (!Number.isFinite(scaledDelta) || Math.abs(scaledDelta) > Number.MAX_SAFE_INTEGER) {
      throw new Error('level scaling intermediate: exceeds the safe arithmetic range');
    }
    unrounded = base + scaledDelta;
    if (!Number.isFinite(unrounded) || Math.abs(unrounded) > Number.MAX_SAFE_INTEGER) {
      throw new Error('level scaling result: must remain finite and within the safe arithmetic range');
    }
  }
