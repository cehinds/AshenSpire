...
function applyUiScale(settings) {
  ...
  // screen. `settingOn` reads the row's own default when the profile has no
  // value — OFF since the owner's uploaded configuration (#1254) — so a profile
  // written before the row existed, and a `?shot=` boot with no durable
  // settings, both follow that default.
