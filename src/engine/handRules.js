...
export function turnDrawCount(ctx, opening = ctx.turn === 1) {
  ...
  ctx.handMax = scaledCards(rules.capacity, ctx.attributes, ctx.ratingAttributeScale || 1);
  const room = Math.max(0, ctx.handMax - ctx.piles.hand.length);
  const wanted = opening ? scaledCards(rules.starting, ctx.attributes, ctx.ratingAttributeScale || 1)
    : rules.drawMode === 'fill' ? room : scaledCards(rules.turn, ctx.attributes, ctx.ratingAttributeScale || 1) + (ctx.pendingDiscardDraw || 0);
...
export function discardChoicePlan(ctx) {
  ...
  const capacity = scaledCards(ctx.handRules.capacity, ctx.attributes, ctx.ratingAttributeScale || 1);
