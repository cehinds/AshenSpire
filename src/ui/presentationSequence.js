...
export function playPresentationSequence(layer,from,context,{targets=[],duration=600,project=activeProject()}={}){
 if(!project||!layer||!from||reducedMotionRequested()||document.body.classList.contains('reduce-flashes'))return ()=>{};
 const resolved=resolveBindings(project,context||{});if(!resolved.winner||resolved.conflict)return ()=>{};
 ...
 let poseOverlay=null,oldVisibility='',token={},raf=0,stopped=false;
 ...
 const stop=()=>{if(stopped)return;stopped=true;cancelAnimationFrame(raf);for(const img of nodes.values())img.remove();poseOverlay?.remove();if(original&&actors.get(original)===stop){original.style.visibility=oldVisibility;actors.delete(original);}};
 ...
 const render=now=>{
  ...
  for(const clip of view.effects){
   ...
   for(let i=0;i<recipients.length;i++){
    ...
    if(target){const tx=target.left+target.width/2,ty=target.top+target.height/2;if(clip.travel){const anchor=project.anchors[clip.anchor]||ANCHORS.hand;const ox=from.left+from.width/2+(anchor[0]-.3)*1000*sx,oy=from.top+from.height/2+(anchor[1]-.55)*600*sy;x=ox+(tx-ox)*clip.progress;y=oy+(ty-oy)*clip.progress;}else{x=tx+clip.x*0+project.clips.find(c=>c.id===clip.id).x*sx;y=ty+project.clips.find(c=>c.id===clip.id).y*sy;}}
