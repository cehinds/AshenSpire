...
export function renderEquipmentCard(registries, piece, { interactive = true } = {}) {
  const model = equipmentCardModel(registries, piece);
  const card = canvasElement();
  card.className = 'equipment-poker-card';
  card.dataset.item = model.id;
  card.setAttribute('aria-label', `${model.name} equipment card`);
  ...
  card.innerHTML = `<span class="epc-frame">
    <span class="epc-name ec-name" ${tip(model.name, 'Painted equipment identity. The card shows authored base values; live upgrades and comparisons are shown separately.')}>${esc(model.name)}<span aria-hidden="true">◆</span></span>
    <span class="epc-art" ${tip(`${model.name} artwork`, 'Painted artwork identifying this equipment. Appearance does not change its mechanics.')}><img src="${esc(equipmentCardArt(piece))}" alt="${esc(model.name)}"></span>
    ...
    <span class="epc-effects ec-mods"><span class="epc-heading">Equipment bonuses</span>${model.bonuses.map(b => `<span class="epc-bonus" ${tip(b.label, b.explanation)}>${esc(b.label)}</span>`).join('') || '<span>No additional bonuses</span>'}</span>
...
export function renderEquipmentInspection(registries, piece) {
  const { card, explanations } = renderEquipmentCard(registries, piece);
