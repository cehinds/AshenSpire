...
export function adoptCombatantFrame(frame) {
  ...
  // The Weapon Art meters ride overhead, where solo's renderPlayer leads with them.
  for (const child of [...frame.children]) (child.matches('.intent, .combatant-info, .art-charge') ? leading : card).append(child);
