...
//
// Every beat fires a COMPOSED cue id through the sink the game already has:
// `sfx.play('beat_holdStart')` and the five below. content/sfx.js resolves an
// id exact-row → FAMILY-ROW → default, splitting at the first underscore — so
// ONE row named `beat` in src/content/sfx.js gives all six a voice with no code
// change here, and six exact rows give each its own. That is the same mechanism
// `procBurst_<status>` uses, which Law 1 clause 7 cites as the shape.
//
// Haptics have ONE home and it is `beatCue` below: a single function, called at
// every boundary, with the action id and the form in hand. Vibration belongs
...
export const BEAT_CUES = Object.freeze([
  'beat_holdStart', 'beat_holdDone', 'beat_holdAbort',
...
export function armHold(btn, { ms, onConfirm, id = null }) {
  ...
  function begin(ev) {
    ...
    trackGesture(ev, {
      onMove: (mv) => {
        if (!armed) return;
        if (Math.hypot(mv.clientX - x0, mv.clientY - y0) > SLOP) { stop('idle'); beatCue('holdAbort', id, 'hold'); }
      },
      // However it ended — lift, cancel, palm, focus loss. If the fill never
      // reached the end, nothing happened, and that IS the feature.
      onEnd: () => { if (armed) { stop('idle'); beatCue('holdAbort', id, 'hold'); } },
    });
  }
  ...
  const onKeyEsc = (ev) => { if (ev.key === 'Escape' && armed) { stop('idle'); beatCue('holdAbort', id, 'hold'); } };
