...
function presentation(registries, item, kind) {
  ...
  return {
    ...
    glyph: potion ? '⚗︎' : '◆',
    identityExplanation: `${item.name}. This card shows the item's authored effect; current quantities are displayed alongside it.`,
    ...
    tags: [{ label: kind, explanation: potion ? 'Stored in a potion slot.' : 'A carried relic.' }],
