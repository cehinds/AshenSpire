...
import { hudQuickSettingsModel, resolveHudMode } from '../models/HudQuickSettingsModel.js';
...
export function runHudViewModel({
  ...
  return componentModel(UI.sharedRunHud, {
    variant: place,
    properties: { place, headerClass, overlayHtml, hudMode: resolveHudMode(quickSettings?.settings) },
    children: [
      ...
      componentModel(UI.hudResizeGrip, {
        variant: place,
        properties: { mode: resolveHudMode(quickSettings?.settings) },
        accessibility: { role: 'button', label: 'Resize run HUD' },
      }),
    ],
  });
}
