...
export function mountCombat(app, { registries, run, combat, label, meta, onEnd, showTutorial, onTutorialDone, onSettings, onMenu, onSave, onQuit }) {
  ...
  /**
   * DOES ENDING THE TURN RIGHT NOW THROW SOMETHING AWAY? One predicate, two
   * readers — the gold pulse that has always said "you still have a play" and
   * the second beat that now makes you hold to give it up. They were never
   * allowed to disagree, so they are not two expressions.
   */
  function endTurnForfeits() {
  ...
  function renderControls() {
    ...
    $('.end-turn').classList.toggle('pulse', endTurnForfeits());
    ...
    // reason a state-dependent beat can live on a control its own screen
    // repaints every frame without any screen tracking the dressing.
