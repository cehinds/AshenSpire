...
export function mountShop(app, { registries, run, meta, onLeave, onChanged, onArmamentPurchased = () => {}, hud = null }) {
  ...
  function render() {
    ...
    stock.relics.forEach((item, i) => {
      ...
      const avail = offerAvailability({ price: item.cost, cinders: run.cinders });
      const tile = shopItem(`${def.icon || '◆'} ${def.name}`, relicText(def, registries), item.cost, avail, { card: renderCollectibleCard(registries, def, 'Relic', { interactive: false }).card });
      addOffer('relics', {
    ...
    stock.flasks.forEach((item, i) => {
      ...
      const tile = shopItem(flaskIdentityHtml(def), def.textTemplate, item.cost, avail, { titleHtml: true, card: renderCollectibleCard(registries, def, 'Potion', { interactive: false }).card });
      addOffer('flasks', {
