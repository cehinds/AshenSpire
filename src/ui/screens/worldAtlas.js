...
export function mountWorldAtlas(
 ...
 <div class="atlas-scrollport" tabindex="0" aria-label="World map; scroll to explore"><div class="atlas-world" style="--atlas-zoom:${j.view?.zoom || 1}"><svg class="atlas-terrain" viewBox="0 0 1000 1000" aria-hidden="true"><defs><filter id="atlas-unmapped"><feGaussianBlur stdDeviation="3"/></filter><radialGradient id="atlas-reveal"><stop offset="60%" stop-color="white"/><stop offset="100%" stop-color="black"/></radialGradient><mask id="atlas-fog"><rect width="1000" height="1000" fill="black"/>${[...known].map((id) => `<circle cx="${pos[id].x * 1000}" cy="${pos[id].y * 1000}" r="${p.revealRadius * 1000}" fill="url(#atlas-reveal)"/>`).join("")}</mask></defs>
 ...
 <aside class="atlas-journal"><span class="atlas-eyebrow">YOUR JOURNEY</span><h2>${esc(current.displayName)}</h2><p>${esc(a.regions[a.regionOf(j.currentNodeId)].displayName)}</p>${button("Inspect current location", "data-atlas-inspect-current")}<div class="atlas-journal-rule"></div><h3>Three guiding lights</h3>${[
  ...
  function inspect(id, returnFocus) {
    ...
    function detail(pointId) {
      ...
      pane.querySelectorAll("[data-local-quest]").forEach(
        (b) =>
          (b.onclick = () => {
            onAction({
              kind: "quest",
      ...
      const g = pane.querySelector("[data-local-gate]");
      if (g)
        g.onclick = () => {
          close();
          onTravel(gate.destinationNodeId);
        };
      const boss = pane.querySelector("[data-local-boss]");
      if (boss)
        boss.onclick = () => {
          close();
          onAction({ kind: "boss", ownerId: id, pointId });
        };
      const visit = pane.querySelector("[data-local-explore]");
      if (visit)
        visit.onclick = () => {
