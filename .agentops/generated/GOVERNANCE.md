...
| authorize-project-schema-change | owner | yes | yes | `target`, `expected_current_hash`, `candidate_oid`, `project_schema_change` | one create-missing-only mutation of the exact authenticated GitHub Project field schema; item content, tickets and backfill remain forbidden |
...
- `scheduler_cutover_binding` — authorize-scheduler-cutover is valid for one attempt before its expiry only. Its structured scheduler_cutover object binds the exact reviewed scheduler head and tree; canonical schema, path, Git blob OID and SHA-256 bindings for the independent QA receipt, Project-schema result, nine-field Project manifest, quiet-window receipt and activation manifest; immutable migration ancestry; exact released-custody current state S0; zero work leases; exact all-off D0 legacy and scheduler-config blobs; and a deterministic activation template. The all-off matrix requires scheduler dispatch false, legacy-watcher authority false, scheduler authorization evidence null, migration dispatch frozen true, legacy activation false, and legacy mode STOOD_DOWN_FOR_SCHEDULER_CUTOVER. The fillable authority input names only existing D0/S0 objects and source evidence; it never predicts A, S1 or D1 identities whose bytes depend on the authority event. The template derives A as the sole child of D0 on only the canonical owner-event, target-capsule and deterministic rendered paths, S1 as the sole child of S0 with exactly one derived activation event, and D1 as the sole child of A with only derived scheduler configuration and canonical cutover evidence changes. A local result receipt records the derived A/S1/D1 OIDs and trees after construction and before publication. The sole publication is one git push --atomic of S1 and D1 with exact compare-and-swap leases against S0 and D0. expected_development_remote_oid always names D0, never A or D1.
