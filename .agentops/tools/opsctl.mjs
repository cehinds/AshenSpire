...
export function runSelftest(root = ROOT) {
  ...
  expectRuntime('exempted lease cannot be widened with an unnamed glob', (rt) => { rt.leases.find((x) => x.id === rt.capsules['AS-1001'].writer_lease).path_globs.push('content/**'); }, 'git-ownership assigns that path to');
  expectRuntime('lease grants an undeclared path glob', (rt) => { const l = rt.leases.find((x) => x.id === rt.capsules['AS-1001'].writer_lease); delete l.path_grant_exception; l.path_globs = ['wildcat/**']; }, 'no git-ownership path declares');
  expectRuntime('lease grants a path owned by a different role', (rt) => { const l = rt.leases.find((x) => x.id === rt.capsules['AS-1001'].writer_lease); delete l.path_grant_exception; l.path_globs = ['.agentops/governance/**']; }, 'git-ownership assigns that path to');
