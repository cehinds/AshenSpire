...
// Deterministic, redacted Owner HUD. A pure function of validated JSON (no
// wall-clock, no secrets, no tokens): the deploying commit SHA is injected at
// publish time by the Pages workflow, so the committed file is drift-gateable.
// The HUD is read-only; decisions go through the owner-command path, never
// through this page. Presentation is delegated wholesale to `renderOwnerPage`
// — the HUD builds body sections only, so it cannot drift off the shared
// owner-page shell the way a hand-rolled document did.
export function renderHud(contracts, rt) {
  ...
  // Same truth-panel shape the Hub overview uses, so the two owner surfaces
  // read as one system. Every number here is the length of a set the HUD
  // already derives below; nothing is counted twice or counted differently.
  const metrics = [
    '<div class="truth-panel" aria-label="Current HUD totals">',
    `<a class="truth-button" href="#traceability"><strong>${tickets.length}</strong><span>Tracked tickets</span><small>validated capsules</small></a>`,
    `<a class="truth-button" href="#needs-you"><strong>${needsYou.length}</strong><span>Owner decisions</span><small>${needsYou.length ? 'decide now' : 'nothing waiting'}</small></a>`,
    `<a class="truth-button" href="#leases"><strong>${activeLeases.length}</strong><span>Writer seats</span><small>active leases</small></a>`,
    `<a class="truth-button blocker-metric" href="#promotion"><strong>${promotion.length}</strong><span>Protected states</span><small>owner-gated to move</small></a>`,
    '</div>',
  ].join('');

  const fold = (id, kicker, heading, blurb, toggle) => `<details class="section-fold" id="${id}" open><summary class="fold-summary"><span class="fold-summary-copy"><span class="kicker">${kicker}</span><strong>${heading}</strong><span>${blurb}</span></span><span class="fold-toggle">${toggle}</span></summary><div class="section-content">`;

  const L = [];

  L.push(fold('needs-you', 'Owner decisions', 'Needs you now', needsYou.length ? `${needsYou.length} decision${needsYou.length === 1 ? '' : 's'} need your authority.` : `Nothing is waiting on ${hubEsc(ownerActor)}.`, needsYou.length ? 'Review' : 'Clear'));
  if (needsYou.length === 0) L.push('<div class="notice"><p class="none">No owner decisions are pending on the current committed state.</p></div>');
  else { L.push('<div class="wrap"><table><tr><th>Ticket</th><th>Why it reached you</th></tr>'); for (const t of needsYou) L.push(`<tr><td><code>${hubEsc(t)}</code></td><td>${hubEsc(dispatchReason.get(t).reason)}</td></tr>`); L.push('</table></div>'); }
  L.push(`<p class="sub">Owner-exclusive command actions: ${ownerReserved.map((a) => `<span class="pill">${hubEsc(a)}</span>`).join(' ')}</p>`);
  L.push('</div></details>');

  L.push(fold('promotion', 'Protected states', 'Promotion candidates &amp; protected risks', promotion.length ? `${promotion.length} ticket${promotion.length === 1 ? ' sits' : 's sit'} at a state only you can move.` : 'Nothing is parked at a protected state.', promotion.length ? 'Review' : 'Clear'));
  if (promotion.length === 0) L.push(`<div class="notice"><p class="none">No ticket is at a protected state (${[...protectedStates].map(hubEsc).join(', ')}) awaiting promotion.</p></div>`);
  else { L.push('<div class="wrap"><table><tr><th>Ticket</th><th>State</th></tr>'); for (const t of promotion) L.push(`<tr><td><code>${hubEsc(t)}</code></td><td><span class="state-pill">${hubEsc(rt.capsules[t].lifecycle_state)}</span></td></tr>`); L.push('</table></div>'); }
  L.push('</div></details>');

  L.push(fold('leases', 'Writer seats', 'Writer leases &amp; collisions', `${activeLeases.length} active lease${activeLeases.length === 1 ? '' : 's'}; one writer per overlapping path or ref.`, 'Inspect'));
  L.push('<div class="wrap"><table><tr><th>Lease</th><th>Actor</th><th>Ticket</th><th>Ref</th><th>Expiry</th></tr>');
  for (const l of activeLeases) L.push(`<tr><td><code>${hubEsc(l.id)}</code></td><td>${hubEsc(l.actor)}</td><td><code>${hubEsc(l.ticket)}</code></td><td><code>${hubEsc(l.ref)}</code></td><td>${hubEsc(l.expiry)}</td></tr>`);
  L.push('</table></div><p class="sub">One writer per overlapping path/ref; collisions are rejected by <code>opsctl verify</code>.</p>');
  L.push('</div></details>');

  L.push(fold('traceability', 'Chain of custody', 'Traceability', 'ticket → actor → task → branch → commit → evidence', 'Trace'));
  L.push('<div class="wrap"><table><tr><th>Ticket</th><th>Owner</th><th>State</th><th>Ref</th><th>Base</th><th>Evidence</th><th>Wake tokens</th></tr>');
  for (const t of tickets) {
    const cap = rt.capsules[t];
    const w = buildCapsule(contracts, rt, t, { frozen: true });
    L.push(`<tr><td><code>${hubEsc(t)}</code></td><td>${hubEsc(cap.owner_actor)}</td><td>${hubEsc(cap.lifecycle_state)}</td><td><code>${hubEsc(cap.ref)}</code></td><td><code>${hubEsc(cap.base_oid.slice(0, 12))}</code></td><td>${cap.evidence_pointers.map(hubEsc).join(', ') || '—'}</td><td>${w.tokens || '—'}</td></tr>`);
  }
  L.push('</table></div>');
  L.push('</div></details>');
  ...
  // Build the raw query; hubEsc() performs the single HTML escape at output.
  const q = (o) => Object.entries(o).map(([k, v]) => `${k}=${encodeURIComponent(v)}`).join('&');
  L.push(fold('decide', 'Owner command', 'Decide', tickets.length ? `${tickets.length} ticket${tickets.length === 1 ? '' : 's'} can be decided against the hash shown.` : 'No tickets are open.', 'Act'));
  if (tickets.length === 0) L.push('<div class="notice"><p class="none">No tickets are open.</p></div>');
  else {
    ...
    for (const t of tickets) {
      ...
      L.push(`<tr><td><code>${hubEsc(t)}</code></td><td>${hubEsc(cap.lifecycle_state)}</td><td><code>${hubEsc(hash.slice(0, 23))}…</code></td><td><a href="${hubEsc(link)}">File a decision →</a></td></tr>`);
    }
    L.push('</table></div>');
  }
  L.push(`<p class="sub">Decisions are enumerated and compare-and-swap-checked: a command filed against a hash that has since moved is rejected as stale, never applied to unseen state. Only the owner's own issues execute.</p>`);
  L.push(`<p><a class="button-link secondary" href="${hubEsc(issueBase)}?${q({ template: 'help-desk-ticket.yml' })}">File a Help Desk ticket →</a></p>`);
  L.push('</div></details>');
  ...
  L.push('<p class="sub">Generated deterministically from <code>.agentops/</code> by <code>opsctl render</code>. No tokens, secrets, or write paths are present. Decisions flow through the authenticated owner-command path, never this page.</p>');

  // The HUD is a single page, so its rail is its own section anchors — every
  // chip goes somewhere that exists. It is not the Hub and does not claim to
  // be one in the topline, and it has no site index to link the brand at.
  return renderOwnerPage(project, 'Owner HUD', '', L.join('\n'), {
    eyebrow: 'Live control surface',
    lede: `A read-only projection of validated repository state — policy ${oi.policy_version}, installed stage ${project.installed_stage} — regenerated by opsctl render, never hand-maintained.`,
    metrics,
    surface: 'Owner HUD',
    home: '',
    navLabel: 'HUD sections',
    nav: [
      { href: '#needs-you', label: 'Needs you now' },
      { href: '#promotion', label: 'Promotion candidates &amp; protected risks' },
      { href: '#leases', label: 'Writer leases &amp; collisions' },
...
export const OWNER_PAGE_CSS = [
  ...
  '@media(max-width:900px){nav{padding-top:22px}.truth-panel{grid-template-columns:repeat(2,minmax(0,1fr))}.ticket-summary{grid-template-columns:minmax(100px,.5fr) minmax(0,1.5fr) auto}.ticket-seat{display:none}.ticket-body{grid-template-columns:1fr}}',
  '@media(max-width:620px){.hero{padding-left:20px;padding-right:20px}.hero-topline .divider,.hero-topline .muted{display:none}.hero-grid,main,.truth-panel,nav,footer{width:min(100% - 28px,1440px)}nav{padding-top:18px}.hero h1{font-size:49px}.truth-panel{grid-template-columns:1fr 1fr}.truth-button{min-height:78px;padding:13px 8px}.truth-button strong{font-size:27px}.fold-summary{grid-template-columns:1fr;gap:12px}.fold-toggle{width:max-content}.ticket-summary{grid-template-columns:1fr auto;gap:7px 10px}.ticket-id{grid-column:1}.ticket-title{grid-column:1/-1}.state-pill{grid-column:2;grid-row:1}.ticket-body{padding:18px}.section-content,section,.panel{padding:20px 16px}dl{grid-template-columns:1fr;gap:3px}dd{margin-bottom:9px}.wrap{margin-left:-8px;margin-right:-8px}}',
  '@media(prefers-reduced-motion:reduce){html{scroll-behavior:auto}}',
].join('');

// The Hub's own rail. It is the default so `renderHubSite` keeps rendering
// exactly what it always has, but the rail is section navigation for the
// surface you are on — a caller that is not the Hub passes its own entries
// rather than inheriting links to pages it does not sit beside.
const HUB_NAV = [
  { href: 'index.html', label: 'Overview' },
  { href: 'decisions.html', label: 'Decisions' },
  { href: 'help-desk.html', label: 'Help desk' },
  { href: 'seats.html', label: 'Seats &amp; teams' },
];

// One shell so every page shares chrome and a reader never lands somewhere with
// no way back. `up` is '' at the site root and '../' one level down. `surface`
// names the site this page belongs to (the topline must not claim a page is the
// Hub when it is not), and `home` is that site's index — a caller with no index
// to point at passes '' and gets an unlinked brand rather than a dead one.
export function renderOwnerPage(project, title, up, bodyHtml, { eyebrow = 'Owner command center', lede = 'Current decisions, accountable work, evidence and routing — projected from validated repository state.', metrics = '', nav = HUB_NAV, navLabel = 'Hub sections', surface = 'Review & Approval Hub', home = 'index.html' } = {}) {
  ...
  // A same-page anchor is already resolved; only a document href is relative to
  // where the page sits in the tree.
  const navLink = ({ href, label }) => `<a href="${href.startsWith('#') ? href : up + href}"${current === href ? ' aria-current="page"' : ''}>${label}</a>`;
  const brandText = `${hubEsc(project.project_name)} · ${hubEsc(surface)}`;
  const brand = home ? `<a class="hero-brand" href="${up}${home}">${brandText}</a>` : `<span class="hero-brand">${brandText}</span>`;
  ...
  return [
    ...
    `${brand}<span class="muted">generated by <code>opsctl render</code></span></div>`,
    '<div class="hero-grid"><div>',
    `<p class="eyebrow">${hubEsc(eyebrow)}</p><h1>${hubEsc(displayTitle)}</h1><p class="lede">${hubEsc(lede)}</p>`,
    '</div></div>',
    // The section chips are their own full-width rail under the metrics strip,
    // not a right-aligned cluster crowding the headline.
    metrics,
    `<nav aria-label="${hubEsc(navLabel)}">`,
    ...nav.map(navLink),
    '</nav>',
    '</header><main>',
    bodyHtml,
