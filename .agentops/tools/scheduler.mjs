...
function verifyCommittedMigrationAuthority(root, payload, source, sourceTree, boundary) {
  ...
  if (binding.source_state_version !== 1 || binding.target_state_version !== 2 || !/^[0-9a-f]{40}$/.test(binding.canonical_anchor_oid ?? '') || runGit(root, ['merge-base', '--is-ancestor', binding.canonical_anchor_oid, source.oid], { allowFailure: true }).status !== 0 || binding.preserved_local_tip_oid !== payload.preserved_local_tip || binding.dispatch_frozen !== true || binding.one_use !== true || binding.target_ref !== 'refs/heads/agentops/scheduler-state' || binding.push_mode !== 'non-force-forward-only-cas' || binding.abort_on_remote_change !== true) throw new Error('migration boundary authority invariants are incomplete');
  validInstant(binding.expires_at, 'scheduler migration authority expires_at');
...
export function main(argv = process.argv.slice(2), root = REPOSITORY_ROOT) {
  ...
  if (command === 'migrate-state') {
    ...
    const preservedLocalTip = localTip && localTip !== remoteOid ? localTip : null;
    const authorityReceipt = validateMigrationAuthority(root, config, state, actualTree, preservedLocalTip, now);
    ...
    state = planStateMigration(state, { expectedOid: remoteOid, expectedTree: actualTree, expectedSnapshotHash: state.snapshot.snapshot_hash, authorityReceipt, preservedLocalTip, createdAt: now });
    ...
    emit(command, { candidate_state_oid: candidate.oid, candidate_ref: candidate.candidateRef, parent_state_oid: remoteOid, preserved_local_tip: preservedLocalTip, source_snapshot_hash: args.expected_snapshot_hash, state_version: '2', dispatch_frozen: true }, 'MIGRATE-STATE CANDIDATE PASS: v1 journal preserved in a local candidate with any divergent local tip retained as a second parent; publication remains separately authorized.'); return 0;
  }
